...
namespace uml4net.xmi.Extensions.EntrepriseArchitect.Structure.Readers
{
    ...
    public partial class ConnectorEndTypeReader : XmiElementReader<IConnectorEndType>, IXmiElementReader<IConnectorEndType>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        ...
        /// </param>
        public ConnectorEndTypeReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, ILoggerFactory loggerFactory)
        ...
        public override IConnectorEndType Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "ConnectorEndType", poco.XmiId);
                }

                poco.Aggregation = xmlReader.GetAttribute("aggregation") ?? xmlReader.GetAttribute("aggregation", this.NameSpaceResolver.UmlNameSpace);
                ...
                while (xmlReader.Read())
                {
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        {
                            case "aggregation":
                                poco.Aggregation = xmlReader.ReadElementContentAsString();
                                break;
                            case "containment":
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"ConnectorEndTypeReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: ConnectorEndTypeReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                                }
