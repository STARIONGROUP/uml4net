...
namespace uml4net.xmi.Extensions.EntrepriseArchitect.Structure.Readers
{
    ...
    public partial class ConnectorEndTypeReader : XmiElementReader<IConnectorEndType>, IXmiElementReader<IConnectorEndType>
    {
        ...
        /// </param>
        public ConnectorEndTypeReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, ILoggerFactory loggerFactory)
        ...
        public override IConnectorEndType Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                poco.Aggregation = xmlReader.GetAttribute("aggregation");
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmlReader.LocalName.LowerCaseFirstLetter())
                        {
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"ConnectorEndTypeReader: {xmlReader.LocalName} at line:position {defaultLineInfo.LineNumber}:{defaultLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.Logger.LogWarning("Not Supported: ConnectorEndTypeReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, defaultLineInfo.LineNumber, defaultLineInfo.LinePosition);
                                }
