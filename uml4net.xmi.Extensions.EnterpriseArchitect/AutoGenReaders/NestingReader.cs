...
namespace uml4net.xmi.Extensions.EnterpriseArchitect.Structure.Readers
{
    ...
    public partial class NestingReader : ExtensionContentReader<INesting>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<NestingReader> logger;
        ...
        /// </param>
        public NestingReader(IExtensionContentReaderFacade extensionContentReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IXmiElementCache cache, ILoggerFactory loggerFactory)
        : base(extensionContentReaderFacade, xmiReaderSettings, nameSpaceResolver, cache, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<NestingReader>.Instance : loggerFactory.CreateLogger<NestingReader>();
        }
        ...
        /// </returns>
        public override INesting Read(XmlReader xmlReader, string documentName)
        {
            if (xmlReader == null)
            {
            ...
            INesting poco = new xmi.Extensions.EnterpriseArchitect.Structure.Nesting();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading Nesting at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmlReader.LocalName)
                        {
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"NestingReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: NestingReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                                }
                                break;
                        }
                    }
