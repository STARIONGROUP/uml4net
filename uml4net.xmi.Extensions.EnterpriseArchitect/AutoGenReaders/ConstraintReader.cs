...
namespace uml4net.xmi.Extensions.EnterpriseArchitect.Structure.Readers
{
    ...
    public partial class ConstraintReader : ExtensionContentReader<IConstraint>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        ...
        /// </param>
        public ConstraintReader(IExtenderReaderRegistry registry, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IXmiElementCache cache, ILoggerFactory loggerFactory)
        ...
        /// </returns>
        public override IConstraint Read(XmlReader xmlReader, string documentName)
        {
            if (xmlReader == null)
            {
            ...
            IConstraint poco = new xmi.Extensions.EnterpriseArchitect.Structure.Constraint();
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                var descriptionValue = xmlReader.GetAttribute("description");
                poco.Description = descriptionValue;
                var nameValue = xmlReader.GetAttribute("name");
                poco.Name = nameValue;
                var notesValue = xmlReader.GetAttribute("notes");
                poco.Notes = notesValue;
                var statusValue = xmlReader.GetAttribute("status");
                if (!string.IsNullOrWhiteSpace(statusValue))
                {
                    poco.Status = (ConstraintStatus)Enum.Parse(typeof(ConstraintStatus), statusValue);
                }

                var typeValue = xmlReader.GetAttribute("type");
                poco.Type = typeValue;
                var weightValue = xmlReader.GetAttribute("weight");
                poco.Weight = weightValue;
