// -------------------------------------------------------------------------------------------------
// <copyright file="ParameterSubstitutionReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Extensions.EntrepriseArchitect.Structure.Readers
{
    ...
    public partial class ParameterSubstitutionReader : XmiElementReader<IParameterSubstitution>, IXmiElementReader<IParameterSubstitution>
    {
        ...
        /// </param>
        public ParameterSubstitutionReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, ILoggerFactory loggerFactory)
        ...
        public override IParameterSubstitution Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                poco.Actual = xmlReader.GetAttribute("actual");

                poco.ActualName = xmlReader.GetAttribute("actualName");
                ...
                poco.Formal = xmlReader.GetAttribute("formal");

                poco.FormalName = xmlReader.GetAttribute("formalName");
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmlReader.LocalName.LowerCaseFirstLetter())
                        {
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"ParameterSubstitutionReader: {xmlReader.LocalName} at line:position {defaultLineInfo.LineNumber}:{defaultLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.Logger.LogWarning("Not Supported: ParameterSubstitutionReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, defaultLineInfo.LineNumber, defaultLineInfo.LinePosition);
                                }
