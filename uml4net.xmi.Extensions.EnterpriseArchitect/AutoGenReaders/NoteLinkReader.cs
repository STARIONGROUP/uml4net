...
namespace uml4net.xmi.Extensions.EntrepriseArchitect.Structure.Readers
{
    ...
    public partial class NoteLinkReader : XmiElementReader<INoteLink>, IXmiElementReader<INoteLink>
    {
        /// <summary>
        ...
        /// </param>
        public NoteLinkReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, ILoggerFactory loggerFactory)
        ...
        /// The name of the document that contains the <see cref="INoteLink"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="INoteLink"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="INoteLink"/>
        /// </returns>
        public override INoteLink Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            INoteLink poco = new xmi.Extensions.EntrepriseArchitect.Structure.NoteLink();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                var xmiType = "NoteLink";
                ...
                {
                    this.Logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "NoteLink", poco.XmiId);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmlReader.LocalName.LowerCaseFirstLetter())
                        {
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"NoteLinkReader: {xmlReader.LocalName} at line:position {defaultLineInfo.LineNumber}:{defaultLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.Logger.LogWarning("Not Supported: NoteLinkReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, defaultLineInfo.LineNumber, defaultLineInfo.LinePosition);
                                }
