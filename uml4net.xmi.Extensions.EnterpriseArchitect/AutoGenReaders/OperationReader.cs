...
namespace uml4net.xmi.Extensions.EnterpriseArchitect.Structure.Readers
{
    ...
    public partial class OperationReader : ExtensionContentReader<IOperation>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        ...
        /// </param>
        public OperationReader(IExtensionContentReaderFacade extensionContentReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IXmiElementCache cache, ILoggerFactory loggerFactory)
        ...
        /// </returns>
        public override IOperation Read(XmlReader xmlReader, string documentName)
        {
            if (xmlReader == null)
            {
            ...
            IOperation poco = new xmi.Extensions.EnterpriseArchitect.Structure.Operation();
                ...
                while (xmlReader.Read())
                {
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmlReader.LocalName)
                        {
                            case "behaviour":
                                {
                                    poco.Behaviour = this.ExtensionContentReaderFacade.QueryExtensionContent<Behaviour>(xmlReader, this.XmiReaderSettings, this.NameSpaceResolver, this.Cache, documentName, this.LoggerFactory);
                                    break;
                                }
                            case "code":
                                {
                            ...
                            case "parameters":
                                {
                                    ...
                                    {
                                        if (parametersReader.NodeType != XmlNodeType.Element || parametersReader.LocalName == "parameters")
                                        {
                                            continue;
                                        }

                                        poco.Parameters.Add(this.ExtensionContentReaderFacade.QueryExtensionContent<Parameter>(parametersReader, this.XmiReaderSettings, this.NameSpaceResolver, this.Cache, documentName, this.LoggerFactory));
                                    }
                            ...
                            case "properties":
                                {
                                    poco.Properties = this.ExtensionContentReaderFacade.QueryExtensionContent<OperationProperties>(xmlReader, this.XmiReaderSettings, this.NameSpaceResolver, this.Cache, documentName, this.LoggerFactory);
                                    break;
                                }
                            case "stereotype":
                                {
                            ...
                            case "type":
                                {
                                    poco.Type = this.ExtensionContentReaderFacade.QueryExtensionContent<TypeElement>(xmlReader, this.XmiReaderSettings, this.NameSpaceResolver, this.Cache, documentName, this.LoggerFactory);
                                    break;
                                }
                            case "xrefs":
                                {
