...
namespace uml4net.Reporting.Generators
{
    ...
    public class ModelInspector : ReportGenerator, IModelInspector
    {
        ...
        public string Inspect(IPackage package, bool includeOperations = false)
        {
            ...
            var classPropertyVariations = MapClassPropertyVariation(package, includeOperations);
        ...
        /// <summary>
        /// Queries the set of variation tokens that characterize the provided <see cref="IOperation"/>,
        /// covering its return type and the type and direction of its arguments
        /// </summary>
        /// <param name="operation">
        /// The <see cref="IOperation"/> that is to be characterized
        /// </param>
        /// <returns>
        /// the variation tokens for the provided <paramref name="operation"/>
        /// </returns>
        private static IEnumerable<string> QueryOperationVariations(IOperation operation)
        {
            var variations = new HashSet<string>();

            var returnParameter = operation.OwnedParameter.FirstOrDefault(x => x.Direction == ParameterDirectionKind.Return);

            variations.Add(returnParameter == null ? "OP:RET:void" : $"OP:RET:{QueryParameterVariation(returnParameter)}");

            foreach (var parameter in operation.OwnedParameter.Where(x => x.Direction != ParameterDirectionKind.Return))
            {
                variations.Add($"OP:PARAM:{parameter.Direction}:{QueryParameterVariation(parameter)}");
            }

            if (operation.IsQuery)
            {
                variations.Add("OP:isQuery");
            }
        ...
        /// <summary>
        /// Queries the variation token (reference, value or enumeration kind together with its
        /// multiplicity) that characterizes the type of the provided <see cref="IParameter"/>
        /// </summary>
        /// <param name="parameter">
        /// The <see cref="IParameter"/> whose type variation is to be queried
        /// </param>
        /// <returns>
        /// a string token characterizing the type and multiplicity of the <paramref name="parameter"/>
        /// </returns>
        private static string QueryParameterVariation(IParameter parameter)
        {
            if (parameter.QueryIsEnum())
            {
                return $"ENUM:{parameter.Lower}:{parameter.Upper}";
            }

            if (parameter.QueryIsValueType())
            {
                return $"VALUE:{parameter.QueryTypeName()}:{parameter.Lower}:{parameter.Upper}";
            }

            return $"REF:{parameter.Lower}:{parameter.Upper}";
        }
        ...
        public void GenerateReport(FileInfo modelPath, DirectoryInfo rootDirectory, string rootPackageXmiId, string rootPackageName,  bool useStrictReading, Dictionary<string, string> pathMap, FileInfo outputPath, string customContent = "")
        {
            ...
            {
                result.Append(this.Inspect(package, this.IncludeOperations));
                result.AppendLine();
