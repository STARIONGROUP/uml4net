// -------------------------------------------------------------------------------------------------
// <copyright file="DurationConstraintReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="DurationConstraintReader"/> is to read an instance of <see cref="IDurationConstraint"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class DurationConstraintReader : XmiElementReader<IDurationConstraint>, IXmiElementReader<IDurationConstraint>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<DurationConstraintReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="DurationConstraintReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public DurationConstraintReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<DurationConstraintReader>.Instance : loggerFactory.CreateLogger<DurationConstraintReader>();
        }

        /// <summary>
        /// Reads the <see cref="IDurationConstraint"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IDurationConstraint"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IDurationConstraint"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IDurationConstraint"/>
        /// </returns>
        public override IDurationConstraint Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IDurationConstraint poco = new DurationConstraint();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading DurationConstraint at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.ResolveQualifiedName(xmlReader.GetXmiAttribute("type"), this.NameSpaceResolver);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:DurationConstraint")
                {
                    throw new XmlException($"The XmiType should be 'uml:DurationConstraint' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:DurationConstraint";
                }
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: DurationConstraint [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                ...
                while (xmlReader.Read())
                {
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        {
                            case (KnowNamespacePrefixes.Uml, "constrainedElement"):
                                TryCollectMultiValueReferencePropertyIdentifiers(xmlReader, poco, "constrainedElement");
                                break;
                            case (KnowNamespacePrefixes.Uml, "context"):
                                CollectSingleValueReferencePropertyIdentifier(xmlReader, poco, "context");
                                break;
                            case (KnowNamespacePrefixes.Uml, "firstEvent"):
                                var firstEventValue = xmlReader.ReadElementContentAsStringInPlace();
                            ...
                            case (KnowNamespacePrefixes.Uml, "qualifiedName"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of DurationConstraint at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"DurationConstraintReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: DurationConstraintReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
