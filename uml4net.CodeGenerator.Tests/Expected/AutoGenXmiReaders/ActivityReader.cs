...
namespace uml4net.xmi.Readers
{
    ...
    public class ActivityReader : XmiElementReader<IActivity>, IXmiElementReader<IActivity>
    {
        ...
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<ActivityReader> logger;
        ...
        /// </param>
        public ActivityReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<ActivityReader>.Instance : loggerFactory.CreateLogger<ActivityReader>();
        }
        ...
        /// The name of the document that contains the <see cref="IActivity"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IActivity"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IActivity"/>
        /// </returns>
        public override IActivity Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            {
                this.logger.LogTrace("reading Activity at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: Activity [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                ...
                var isSingleExecutionXmlAttribute = xmlReader.GetAttribute("isSingleExecution") ?? xmlReader.GetAttribute("isSingleExecution", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(isSingleExecutionXmlAttribute))
                {
                    poco.IsSingleExecution = XmlConvert.ToBoolean(isSingleExecutionXmlAttribute);
                }
                ...
                var partitionXmlAttribute = xmlReader.GetAttribute("partition") ?? xmlReader.GetAttribute("partition", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(partitionXmlAttribute))
                {
                ...
                var redefinedBehaviorXmlAttribute = xmlReader.GetAttribute("redefinedBehavior") ?? xmlReader.GetAttribute("redefinedBehavior", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(redefinedBehaviorXmlAttribute))
                {
                ...
                var structuredNodeXmlAttribute = xmlReader.GetAttribute("structuredNode") ?? xmlReader.GetAttribute("structuredNode", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(structuredNodeXmlAttribute))
                {
                    var structuredNodeXmlAttributeValues = structuredNodeXmlAttribute.Split(SplitMultiReference, StringSplitOptions.RemoveEmptyEntries).ToList();
                    poco.MultiValueReferencePropertyIdentifiers.Add("structuredNode", structuredNodeXmlAttributeValues);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "collaborationUse"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "collaborationUse", poco.CollaborationUse.Count))
                                {
                            ...
                            case (KnowNamespacePrefixes.Uml, "edge"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "edge", poco.Edge.Count))
                                {
                            ...
                            case (KnowNamespacePrefixes.Uml, "generalization"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "generalization", poco.Generalization.Count))
                                {
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "group"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "group", poco.Group.Count))
                                {
                                    var groupValue = (IActivityGroup)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.ExtenderReaderRegistry, this.LoggerFactory);
                                    poco.Group.Add(groupValue);
                                }
                                break;
                            case (KnowNamespacePrefixes.Uml, "interfaceRealization"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "interfaceRealization", poco.InterfaceRealization.Count))
                                {
                            ...
                            case (KnowNamespacePrefixes.Uml, "isReadOnly"):
                                var isReadOnlyValue = xmlReader.ReadElementContentAsStringInPlace();
                            ...
                            case (KnowNamespacePrefixes.Uml, "isReentrant"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "isSingleExecution"):
                                var isSingleExecutionValue = xmlReader.ReadElementContentAsStringInPlace();

                                if (!string.IsNullOrWhiteSpace(isSingleExecutionValue))
                                {
                                    poco.IsSingleExecution = XmlConvert.ToBoolean(isSingleExecutionValue);
                                }
                            ...
                            case (KnowNamespacePrefixes.Uml, "nestedClassifier"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "nestedClassifier", poco.NestedClassifier.Count))
                                {
                                    var nestedClassifierValue = (IClassifier)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.ExtenderReaderRegistry, this.LoggerFactory);
                                    poco.NestedClassifier.Add(nestedClassifierValue);
                                }
                                break;
                            case (KnowNamespacePrefixes.Uml, "node"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "node", poco.Node.Count))
                                {
                            ...
                            case (KnowNamespacePrefixes.Uml, "ownedAttribute"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "ownedAttribute", poco.OwnedAttribute.Count))
                                {
                                    var ownedAttributeValue = (IProperty)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.ExtenderReaderRegistry, this.LoggerFactory, "uml:Property");
                                    poco.OwnedAttribute.Add(ownedAttributeValue);
                                }
                                break;
                            case (KnowNamespacePrefixes.Uml, "ownedBehavior"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "ownedBehavior", poco.OwnedBehavior.Count))
                                {
                            ...
                            case (KnowNamespacePrefixes.Uml, "ownedConnector"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "ownedConnector", poco.OwnedConnector.Count))
                                {
                                    var ownedConnectorValue = (IConnector)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.ExtenderReaderRegistry, this.LoggerFactory, "uml:Connector");
                                    poco.OwnedConnector.Add(ownedConnectorValue);
                                }
                                break;
                            case (KnowNamespacePrefixes.Uml, "ownedOperation"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "ownedOperation", poco.OwnedOperation.Count))
                                {
                                    var ownedOperationValue = (IOperation)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.ExtenderReaderRegistry, this.LoggerFactory, "uml:Operation");
                                    poco.OwnedOperation.Add(ownedOperationValue);
                                }
                                break;
                            case (KnowNamespacePrefixes.Uml, "ownedParameter"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "ownedParameter", poco.OwnedParameter.Count))
                                {
                            ...
                            case (KnowNamespacePrefixes.Uml, "ownedParameterSet"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "ownedParameterSet", poco.OwnedParameterSet.Count))
                                {
                            ...
                            case (KnowNamespacePrefixes.Uml, "ownedReception"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "ownedReception", poco.OwnedReception.Count))
                                {
                                    var ownedReceptionValue = (IReception)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.ExtenderReaderRegistry, this.LoggerFactory, "uml:Reception");
                            ...
                            case (KnowNamespacePrefixes.Uml, "ownedUseCase"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "ownedUseCase", poco.OwnedUseCase.Count))
                                {
                            ...
                            case (KnowNamespacePrefixes.Uml, "packageImport"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "partition"):
                                TryCollectMultiValueReferencePropertyIdentifiers(xmlReader, poco, "partition");
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "precondition"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "redefinedBehavior"):
                                TryCollectMultiValueReferencePropertyIdentifiers(xmlReader, poco, "redefinedBehavior");
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "specification"):
                                CollectSingleValueReferencePropertyIdentifier(xmlReader, poco, "specification");
                                break;
                            case (KnowNamespacePrefixes.Uml, "structuredNode"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "structuredNode", poco.StructuredNode.Count))
                                {
                                    var structuredNodeValue = (IStructuredActivityNode)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.ExtenderReaderRegistry, this.LoggerFactory, "uml:StructuredActivityNode");
                                    poco.StructuredNode.Add(structuredNodeValue);
                                }
                                break;
                            case (KnowNamespacePrefixes.Uml, "substitution"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "substitution", poco.Substitution.Count))
                                {
                            ...
                            case (KnowNamespacePrefixes.Uml, "templateBinding"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "templateBinding", poco.TemplateBinding.Count))
                                {
                            ...
                            case (KnowNamespacePrefixes.Uml, "variable"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "variable", poco.Variable.Count))
                                {
                            ...
                            case (KnowNamespacePrefixes.Uml, "visibility"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "attribute"):
                            ...
                            case (KnowNamespacePrefixes.Uml, "feature"):
                            case (KnowNamespacePrefixes.Uml, "general"):
                            case (KnowNamespacePrefixes.Uml, "importedMember"):
                            case (KnowNamespacePrefixes.Uml, "inheritedMember"):
                            ...
                            case (KnowNamespacePrefixes.Uml, "ownedPort"):
                            case (KnowNamespacePrefixes.Uml, "owner"):
                            case (KnowNamespacePrefixes.Uml, "part"):
                            ...
                            case (KnowNamespacePrefixes.Uml, "superClass"):
                                // serialized derived data (XMI 2.5.1 clause 7.8.10) is computed by uml4net, not read
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of Activity at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"ActivityReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: ActivityReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
