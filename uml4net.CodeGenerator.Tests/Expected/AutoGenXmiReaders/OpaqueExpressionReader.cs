...
namespace uml4net.xmi.Readers
{
    ...
    public class OpaqueExpressionReader : XmiElementReader<IOpaqueExpression>, IXmiElementReader<IOpaqueExpression>
    {
        ...
        /// </param>
        public OpaqueExpressionReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<OpaqueExpressionReader>.Instance : loggerFactory.CreateLogger<OpaqueExpressionReader>();
        }
        ...
        /// The name of the document that contains the <see cref="IOpaqueExpression"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IOpaqueExpression"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IOpaqueExpression"/>
        /// </returns>
        public override IOpaqueExpression Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: OpaqueExpression [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "result"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of OpaqueExpression at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    this.logger.LogWarning("Not Supported: OpaqueExpressionReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
