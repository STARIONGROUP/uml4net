...
namespace uml4net.xmi.Readers
{
    ...
    public class ConnectorReader : XmiElementReader<IConnector>, IXmiElementReader<IConnector>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        ...
        /// </param>
        public ConnectorReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
        ...
        public override IConnector Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:Connector")
                {
                    throw new XmlException($"The XmiType should be 'uml:Connector' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:Connector";
                }
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: Connector [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                ...
                {
                    var contractXmlAttributeValues = contractXmlAttribute.Split(SplitMultiReference, StringSplitOptions.RemoveEmptyEntries).ToList();
                    poco.MultiValueReferencePropertyIdentifiers.Add("contract", contractXmlAttributeValues);
                }
                ...
                var redefinedConnectorXmlAttribute = xmlReader.GetAttribute("redefinedConnector") ?? xmlReader.GetAttribute("redefinedConnector", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(redefinedConnectorXmlAttribute))
                {
                    var redefinedConnectorXmlAttributeValues = redefinedConnectorXmlAttribute.Split(SplitMultiReference, StringSplitOptions.RemoveEmptyEntries).ToList();
                    poco.MultiValueReferencePropertyIdentifiers.Add("redefinedConnector", redefinedConnectorXmlAttributeValues);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "end"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "end", poco.End.Count))
                                {
                            ...
                            case (KnowNamespacePrefixes.Uml, "redefinitionContext"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of Connector at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
