...
namespace uml4net.xmi.Readers
{
    ...
    public class StateMachineReader : XmiElementReader<IStateMachine>, IXmiElementReader<IStateMachine>
    {
        ...
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<StateMachineReader> logger;
        ...
        /// </param>
        public StateMachineReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<StateMachineReader>.Instance : loggerFactory.CreateLogger<StateMachineReader>();
        }

        /// <summary>
        /// Reads the <see cref="IStateMachine"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IStateMachine"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IStateMachine"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IStateMachine"/>
        /// </returns>
        public override IStateMachine Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:StateMachine")
                {
                    throw new XmlException($"The XmiType should be 'uml:StateMachine' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:StateMachine";
                }
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: StateMachine [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "connectionPoint"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "connectionPoint", poco.ConnectionPoint.Count))
                                {
                            ...
                            case (KnowNamespacePrefixes.Uml, "extendedStateMachine"):
                                TryCollectMultiValueReferencePropertyIdentifiers(xmlReader, poco, "extendedStateMachine");
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "region"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "region", poco.Region.Count))
                                {
                            ...
                            case (KnowNamespacePrefixes.Uml, "submachineState"):
                                TryCollectMultiValueReferencePropertyIdentifiers(xmlReader, poco, "submachineState");
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "superClass"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of StateMachine at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    this.logger.LogWarning("Not Supported: StateMachineReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
