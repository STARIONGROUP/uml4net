...
namespace uml4net.xmi.Readers
{
    ...
    public class PropertyReader : XmiElementReader<IProperty>, IXmiElementReader<IProperty>
    {
        ...
        /// </param>
        public PropertyReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<PropertyReader>.Instance : loggerFactory.CreateLogger<PropertyReader>();
        }
        ...
        /// The name of the document that contains the <see cref="IProperty"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IProperty"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IProperty"/>
        /// </returns>
        public override IProperty Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "upper"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of Property at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    this.logger.LogWarning("Not Supported: PropertyReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
