// -------------------------------------------------------------------------------------------------
// <copyright file="TimeConstraintReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    public class TimeConstraintReader : XmiElementReader<ITimeConstraint>, IXmiElementReader<ITimeConstraint>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<TimeConstraintReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="TimeConstraintReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public TimeConstraintReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<TimeConstraintReader>.Instance : loggerFactory.CreateLogger<TimeConstraintReader>();
        }

        /// <summary>
        /// Reads the <see cref="ITimeConstraint"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="ITimeConstraint"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="ITimeConstraint"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="ITimeConstraint"/>
        /// </returns>
        public override ITimeConstraint Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            {
                this.logger.LogTrace("reading TimeConstraint at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.ResolveQualifiedName(xmlReader.GetXmiAttribute("type"), this.NameSpaceResolver);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:TimeConstraint")
                {
                    throw new XmlException($"The XmiType should be 'uml:TimeConstraint' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:TimeConstraint";
                }
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: TimeConstraint [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "specification"):
                                ...
                                {
                                    var specificationValue = (ITimeInterval)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.ExtenderReaderRegistry, this.LoggerFactory, "uml:TimeInterval");
                            ...
                            case (KnowNamespacePrefixes.Uml, "qualifiedName"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of TimeConstraint at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"TimeConstraintReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: TimeConstraintReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
