...
namespace uml4net.xmi.Readers
{
    ...
    public class ClearAssociationActionReader : XmiElementReader<IClearAssociationAction>, IXmiElementReader<IClearAssociationAction>
    {
        ...
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<ClearAssociationActionReader> logger;
        ...
        /// </param>
        public ClearAssociationActionReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<ClearAssociationActionReader>.Instance : loggerFactory.CreateLogger<ClearAssociationActionReader>();
        }
        ...
        /// The name of the document that contains the <see cref="IClearAssociationAction"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IClearAssociationAction"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IClearAssociationAction"/>
        /// </returns>
        public override IClearAssociationAction Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            {
                this.logger.LogTrace("reading ClearAssociationAction at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "ClearAssociationAction", poco.XmiId);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"ClearAssociationActionReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: ClearAssociationActionReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                                }
