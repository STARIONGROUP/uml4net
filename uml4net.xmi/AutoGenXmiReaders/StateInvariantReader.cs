...
namespace uml4net.xmi.Readers
{
    ...
    public class StateInvariantReader : XmiElementReader<IStateInvariant>, IXmiElementReader<IStateInvariant>
    {
        ...
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<StateInvariantReader> logger;
        ...
        /// </param>
        public StateInvariantReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<StateInvariantReader>.Instance : loggerFactory.CreateLogger<StateInvariantReader>();
        }
        ...
        /// The name of the document that contains the <see cref="IStateInvariant"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IStateInvariant"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IStateInvariant"/>
        /// </returns>
        public override IStateInvariant Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            {
                this.logger.LogTrace("reading StateInvariant at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: StateInvariant [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "generalOrdering"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "invariant"):
                            ...
                            case (KnowNamespacePrefixes.Uml, "qualifiedName"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of StateInvariant at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"StateInvariantReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: StateInvariantReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
