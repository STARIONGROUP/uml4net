        ...
        public override IUseCase Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: UseCase [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "extend"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "extend", poco.Extend.Count))
                                {
                            ...
                            case (KnowNamespacePrefixes.Uml, "extensionPoint"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "extensionPoint", poco.ExtensionPoint.Count))
                                {
                            ...
                            case (KnowNamespacePrefixes.Uml, "include"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "include", poco.Include.Count))
                                {
                            ...
                            case (KnowNamespacePrefixes.Uml, "subject"):
                                TryCollectMultiValueReferencePropertyIdentifiers(xmlReader, poco, "subject");
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "redefinitionContext"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of UseCase at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
