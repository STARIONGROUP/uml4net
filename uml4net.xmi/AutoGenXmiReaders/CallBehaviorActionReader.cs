// -------------------------------------------------------------------------------------------------
// <copyright file="CallBehaviorActionReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    public class CallBehaviorActionReader : XmiElementReader<ICallBehaviorAction>, IXmiElementReader<ICallBehaviorAction>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<CallBehaviorActionReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="CallBehaviorActionReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public CallBehaviorActionReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<CallBehaviorActionReader>.Instance : loggerFactory.CreateLogger<CallBehaviorActionReader>();
        }

        /// <summary>
        /// Reads the <see cref="ICallBehaviorAction"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="ICallBehaviorAction"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="ICallBehaviorAction"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="ICallBehaviorAction"/>
        /// </returns>
        public override ICallBehaviorAction Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            ICallBehaviorAction poco = new CallBehaviorAction();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading CallBehaviorAction at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.GetAttribute("type", this.NameSpaceResolver.XmiNameSpace);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:CallBehaviorAction")
                {
                    throw new XmlException($"The XmiType should be 'uml:CallBehaviorAction' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:CallBehaviorAction";
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "CallBehaviorAction", poco.XmiId);
                }
                ...
                var behaviorXmlAttribute = xmlReader.GetAttribute("behavior") ?? xmlReader.GetAttribute("behavior", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(behaviorXmlAttribute))
                {
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"CallBehaviorActionReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: CallBehaviorActionReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                                }
