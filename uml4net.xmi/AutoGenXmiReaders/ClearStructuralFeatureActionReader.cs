// -------------------------------------------------------------------------------------------------
// <copyright file="ClearStructuralFeatureActionReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="ClearStructuralFeatureActionReader"/> is to read an instance of <see cref="IClearStructuralFeatureAction"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class ClearStructuralFeatureActionReader : XmiElementReader<IClearStructuralFeatureAction>, IXmiElementReader<IClearStructuralFeatureAction>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<ClearStructuralFeatureActionReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="ClearStructuralFeatureActionReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public ClearStructuralFeatureActionReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<ClearStructuralFeatureActionReader>.Instance : loggerFactory.CreateLogger<ClearStructuralFeatureActionReader>();
        }

        /// <summary>
        /// Reads the <see cref="IClearStructuralFeatureAction"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IClearStructuralFeatureAction"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IClearStructuralFeatureAction"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IClearStructuralFeatureAction"/>
        /// </returns>
        public override IClearStructuralFeatureAction Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IClearStructuralFeatureAction poco = new ClearStructuralFeatureAction();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading ClearStructuralFeatureAction at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.GetAttribute("type", this.NameSpaceResolver.XmiNameSpace);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:ClearStructuralFeatureAction")
                {
                    throw new XmlException($"The XmiType should be 'uml:ClearStructuralFeatureAction' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:ClearStructuralFeatureAction";
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "ClearStructuralFeatureAction", poco.XmiId);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"ClearStructuralFeatureActionReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: ClearStructuralFeatureActionReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                                }
