// -------------------------------------------------------------------------------------------------
// <copyright file="TestIdentityActionReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="TestIdentityActionReader"/> is to read an instance of <see cref="ITestIdentityAction"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class TestIdentityActionReader : XmiElementReader<ITestIdentityAction>, IXmiElementReader<ITestIdentityAction>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<TestIdentityActionReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="TestIdentityActionReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public TestIdentityActionReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<TestIdentityActionReader>.Instance : loggerFactory.CreateLogger<TestIdentityActionReader>();
        }

        /// <summary>
        /// Reads the <see cref="ITestIdentityAction"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="ITestIdentityAction"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="ITestIdentityAction"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="ITestIdentityAction"/>
        /// </returns>
        public override ITestIdentityAction Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            ITestIdentityAction poco = new TestIdentityAction();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading TestIdentityAction at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.GetAttribute("type", this.NameSpaceResolver.XmiNameSpace);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:TestIdentityAction")
                {
                    throw new XmlException($"The XmiType should be 'uml:TestIdentityAction' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:TestIdentityAction";
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "TestIdentityAction", poco.XmiId);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            case (KnowNamespacePrefixes.Uml, "activity"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "first"):
                                var firstValue = (IInputPin)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.LoggerFactory, "uml:InputPin");
                            ...
                            case (KnowNamespacePrefixes.Uml, "second"):
                                var secondValue = (IInputPin)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.LoggerFactory, "uml:InputPin");
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"TestIdentityActionReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: TestIdentityActionReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                                }
