...
namespace uml4net.xmi.Readers.Activities
{
    ...
    public class JoinNodeReader : XmiElementReader<IJoinNode>, IXmiElementReader<IJoinNode>
    {
        ...
        /// </param>
        public JoinNodeReader(IXmiReaderCache cache, ILoggerFactory loggerFactory)
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "isCombineDuplicate":
                                var isCombineDuplicateXmlElement = xmlReader.ReadElementContentAsString();
                                if (!string.IsNullOrEmpty(isCombineDuplicateXmlElement))
                                {
                                    poco.IsCombineDuplicate = bool.Parse(isCombineDuplicateXmlElement);
                                }
                                break;
                            case "isLeaf":
                                ...
                                break;
                            case "joinSpec":
                                var joinSpec = (IValueSpecification)this.xmiElementReaderFacade.QueryXmiElement(xmlReader, this.Cache, this.LoggerFactory);
                                poco.JoinSpec.Add(joinSpec);
                                break;
                            case "name":
