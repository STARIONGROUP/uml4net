// -------------------------------------------------------------------------------------------------
// <copyright file="JoinNodeReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="JoinNodeReader"/> is to read an instance of <see cref="IJoinNode"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class JoinNodeReader : XmiElementReader<IJoinNode>, IXmiElementReader<IJoinNode>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<JoinNodeReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="JoinNodeReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public JoinNodeReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<JoinNodeReader>.Instance : loggerFactory.CreateLogger<JoinNodeReader>();
        }

        /// <summary>
        /// Reads the <see cref="IJoinNode"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IJoinNode"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IJoinNode"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IJoinNode"/>
        /// </returns>
        public override IJoinNode Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IJoinNode poco = new JoinNode();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading JoinNode at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.ResolveQualifiedName(xmlReader.GetXmiAttribute("type"), this.NameSpaceResolver);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:JoinNode")
                {
                    throw new XmlException($"The XmiType should be 'uml:JoinNode' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:JoinNode";
                }
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: JoinNode [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                ...
                var isCombineDuplicateXmlAttribute = xmlReader.GetAttribute("isCombineDuplicate") ?? xmlReader.GetAttribute("isCombineDuplicate", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(isCombineDuplicateXmlAttribute))
                {
                    poco.IsCombineDuplicate = XmlConvert.ToBoolean(isCombineDuplicateXmlAttribute);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "inStructuredNode"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "isCombineDuplicate"):
                                var isCombineDuplicateValue = xmlReader.ReadElementContentAsStringInPlace();

                                if (!string.IsNullOrWhiteSpace(isCombineDuplicateValue))
                                {
                                    poco.IsCombineDuplicate = XmlConvert.ToBoolean(isCombineDuplicateValue);
                                }
                            ...
                            case (KnowNamespacePrefixes.Uml, "isLeaf"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "joinSpec"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "joinSpec", poco.JoinSpec.Count))
                                {
                                    var joinSpecValue = (IValueSpecification)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.ExtenderReaderRegistry, this.LoggerFactory);
                                    poco.JoinSpec.Add(joinSpecValue);
                                }
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "redefinitionContext"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of JoinNode at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"JoinNodeReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: JoinNodeReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
