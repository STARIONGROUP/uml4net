...
namespace uml4net.xmi.Readers
{
    ...
    public class DeploymentSpecificationReader : XmiElementReader<IDeploymentSpecification>, IXmiElementReader<IDeploymentSpecification>
    {
        ...
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<DeploymentSpecificationReader> logger;
        ...
        /// </param>
        public DeploymentSpecificationReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<DeploymentSpecificationReader>.Instance : loggerFactory.CreateLogger<DeploymentSpecificationReader>();
        }
        ...
        /// The name of the document that contains the <see cref="IDeploymentSpecification"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IDeploymentSpecification"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IDeploymentSpecification"/>
        /// </returns>
        public override IDeploymentSpecification Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            {
                this.logger.LogTrace("reading DeploymentSpecification at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: DeploymentSpecification [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }

                var deploymentXmlAttribute = xmlReader.GetAttribute("deployment") ?? xmlReader.GetAttribute("deployment", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(deploymentXmlAttribute))
                {
                    poco.SingleValueReferencePropertyIdentifiers.Add("deployment", deploymentXmlAttribute);
                }

                poco.DeploymentLocation = xmlReader.GetAttribute("deploymentLocation") ?? xmlReader.GetAttribute("deploymentLocation", this.NameSpaceResolver.UmlNameSpace);

                poco.ExecutionLocation = xmlReader.GetAttribute("executionLocation") ?? xmlReader.GetAttribute("executionLocation", this.NameSpaceResolver.UmlNameSpace);

                poco.FileName = xmlReader.GetAttribute("fileName") ?? xmlReader.GetAttribute("fileName", this.NameSpaceResolver.UmlNameSpace);
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "deployment"):
                                CollectSingleValueReferencePropertyIdentifier(xmlReader, poco, "deployment");
                                break;
                            case (KnowNamespacePrefixes.Uml, "deploymentLocation"):
                                poco.DeploymentLocation = xmlReader.ReadElementContentAsStringInPlace();
                                break;
                            case (KnowNamespacePrefixes.Uml, "elementImport"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "executionLocation"):
                                poco.ExecutionLocation = xmlReader.ReadElementContentAsStringInPlace();
                                break;
                            case (KnowNamespacePrefixes.Uml, "fileName"):
                                poco.FileName = xmlReader.ReadElementContentAsStringInPlace();
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "isLeaf"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "manifestation"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "manifestation", poco.Manifestation.Count))
                                {
                                    var manifestationValue = (IManifestation)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.ExtenderReaderRegistry, this.LoggerFactory, "uml:Manifestation");
                            ...
                            case (KnowNamespacePrefixes.Uml, "nameExpression"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "nestedArtifact"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "nestedArtifact", poco.NestedArtifact.Count))
                                {
                                    var nestedArtifactValue = (IArtifact)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.ExtenderReaderRegistry, this.LoggerFactory, "uml:Artifact");
                            ...
                            case (KnowNamespacePrefixes.Uml, "redefinitionContext"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of DeploymentSpecification at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"DeploymentSpecificationReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: DeploymentSpecificationReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
