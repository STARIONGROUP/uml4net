...
namespace uml4net.xmi.Readers
{
    ...
    public class TimeEventReader : XmiElementReader<ITimeEvent>, IXmiElementReader<ITimeEvent>
    {
        ...
        /// </param>
        public TimeEventReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, ILoggerFactory loggerFactory)
        ...
        public override ITimeEvent Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "TimeEvent", poco.XmiId);
                }
                ...
                if (!string.IsNullOrEmpty(isRelativeXmlAttribute))
                {
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            case (KnowNamespacePrefixes.Uml, "isRelative"):
                                var isRelativeValue = xmlReader.ReadElementContentAsString();

                                if (!string.IsNullOrEmpty(isRelativeValue))
                                {
                            ...
                            case (KnowNamespacePrefixes.Uml, "when"):
                                var whenValue = (ITimeExpression)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.LoggerFactory, "uml:TimeExpression");
