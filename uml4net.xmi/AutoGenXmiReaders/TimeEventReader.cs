...
namespace uml4net.xmi.Readers.CommonBehavior
{
    ...
    public class TimeEventReader : XmiElementReader<ITimeEvent>, IXmiElementReader<ITimeEvent>
    {
        ...
        /// </param>
        public TimeEventReader(IXmiReaderCache cache, ILoggerFactory loggerFactory)
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                while (xmlReader.Read())
                {
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        {
                            case "isRelative":
                                var isRelativeXmlElement = xmlReader.ReadElementContentAsString();
                                if (!string.IsNullOrEmpty(isRelativeXmlElement))
                                {
                                    poco.IsRelative = bool.Parse(isRelativeXmlElement);
                                }
                                break;
                            case "name":
                            ...
                            case "visibility":
                                ...
                                break;
                            case "when":
                                var when = (ITimeExpression)this.xmiElementReaderFacade.QueryXmiElement(xmlReader, this.Cache, this.LoggerFactory, "uml:TimeExpression");
                                poco.When.Add(when);
                                break;
                            default:
