...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="TimeEventReader"/> is to read an instance of <see cref="ITimeEvent"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class TimeEventReader : XmiElementReader<ITimeEvent>, IXmiElementReader<ITimeEvent>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<TimeEventReader> logger;
        ...
        /// </param>
        public TimeEventReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<TimeEventReader>.Instance : loggerFactory.CreateLogger<TimeEventReader>();
        }

        /// <summary>
        /// Reads the <see cref="ITimeEvent"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="ITimeEvent"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="ITimeEvent"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="ITimeEvent"/>
        /// </returns>
        public override ITimeEvent Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:TimeEvent")
                {
                    throw new XmlException($"The XmiType should be 'uml:TimeEvent' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:TimeEvent";
                }
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: TimeEvent [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }

                var isRelativeXmlAttribute = xmlReader.GetAttribute("isRelative") ?? xmlReader.GetAttribute("isRelative", this.NameSpaceResolver.UmlNameSpace);
                ...
                while (xmlReader.Read())
                {
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        {
                            case (KnowNamespacePrefixes.Uml, "isRelative"):
                                var isRelativeValue = xmlReader.ReadElementContentAsStringInPlace();
                            ...
                            case (KnowNamespacePrefixes.Uml, "qualifiedName"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of TimeEvent at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    this.logger.LogWarning("Not Supported: TimeEventReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
