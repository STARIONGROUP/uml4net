// -------------------------------------------------------------------------------------------------
// <copyright file="CentralBufferNodeReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="CentralBufferNodeReader"/> is to read an instance of <see cref="ICentralBufferNode"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class CentralBufferNodeReader : XmiElementReader<ICentralBufferNode>, IXmiElementReader<ICentralBufferNode>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<CentralBufferNodeReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="CentralBufferNodeReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public CentralBufferNodeReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<CentralBufferNodeReader>.Instance : loggerFactory.CreateLogger<CentralBufferNodeReader>();
        }

        /// <summary>
        /// Reads the <see cref="ICentralBufferNode"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="ICentralBufferNode"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="ICentralBufferNode"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="ICentralBufferNode"/>
        /// </returns>
        public override ICentralBufferNode Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            ICentralBufferNode poco = new CentralBufferNode();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading CentralBufferNode at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.ResolveQualifiedName(xmlReader.GetXmiAttribute("type"), this.NameSpaceResolver);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:CentralBufferNode")
                {
                    throw new XmlException($"The XmiType should be 'uml:CentralBufferNode' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:CentralBufferNode";
                }
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: CentralBufferNode [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "redefinitionContext"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of CentralBufferNode at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"CentralBufferNodeReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: CentralBufferNodeReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
