// -------------------------------------------------------------------------------------------------
// <copyright file="SignalEventReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="SignalEventReader"/> is to read an instance of <see cref="ISignalEvent"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class SignalEventReader : XmiElementReader<ISignalEvent>, IXmiElementReader<ISignalEvent>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<SignalEventReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="SignalEventReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public SignalEventReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<SignalEventReader>.Instance : loggerFactory.CreateLogger<SignalEventReader>();
        }

        /// <summary>
        /// Reads the <see cref="ISignalEvent"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="ISignalEvent"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="ISignalEvent"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="ISignalEvent"/>
        /// </returns>
        public override ISignalEvent Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            ISignalEvent poco = new SignalEvent();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading SignalEvent at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.ResolveQualifiedName(xmlReader.GetXmiAttribute("type"), this.NameSpaceResolver);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:SignalEvent")
                {
                    throw new XmlException($"The XmiType should be 'uml:SignalEvent' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:SignalEvent";
                }
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: SignalEvent [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "qualifiedName"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of SignalEvent at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"SignalEventReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: SignalEventReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
