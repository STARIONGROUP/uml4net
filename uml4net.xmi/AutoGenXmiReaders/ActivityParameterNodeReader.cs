// -------------------------------------------------------------------------------------------------
// <copyright file="ActivityParameterNodeReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="ActivityParameterNodeReader"/> is to read an instance of <see cref="IActivityParameterNode"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class ActivityParameterNodeReader : XmiElementReader<IActivityParameterNode>, IXmiElementReader<IActivityParameterNode>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<ActivityParameterNodeReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="ActivityParameterNodeReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public ActivityParameterNodeReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<ActivityParameterNodeReader>.Instance : loggerFactory.CreateLogger<ActivityParameterNodeReader>();
        }

        /// <summary>
        /// Reads the <see cref="IActivityParameterNode"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IActivityParameterNode"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IActivityParameterNode"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IActivityParameterNode"/>
        /// </returns>
        public override IActivityParameterNode Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IActivityParameterNode poco = new ActivityParameterNode();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading ActivityParameterNode at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.ResolveQualifiedName(xmlReader.GetXmiAttribute("type"), this.NameSpaceResolver);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:ActivityParameterNode")
                {
                    throw new XmlException($"The XmiType should be 'uml:ActivityParameterNode' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:ActivityParameterNode";
                }
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: ActivityParameterNode [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                ...
                {
                    if (ObjectNodeOrderingKindExtensions.TryParseXmiLiteral(orderingXmlAttribute, out var orderingLiteral))
                    {
                        poco.Ordering = orderingLiteral;
                    }
                    else
                    {
                        this.ReportXmiError(xmlReader, poco, "ordering", $"[{orderingXmlAttribute}] is not the name of a literal of ObjectNodeOrderingKind");
                    }
                }
                ...
                {
                    poco.SingleValueReferencePropertyIdentifiers.Add("parameter", parameterXmlAttribute);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "isControlType"):
                                var isControlTypeValue = xmlReader.ReadElementContentAsStringInPlace();
                            ...
                            case (KnowNamespacePrefixes.Uml, "ordering"):
                                var orderingValue = xmlReader.ReadElementContentAsStringInPlace();

                                if (!string.IsNullOrWhiteSpace(orderingValue))
                                {
                                    if (ObjectNodeOrderingKindExtensions.TryParseXmiLiteral(orderingValue, out var orderingLiteral))
                                    {
                                        poco.Ordering = orderingLiteral;
                                    }
                                    else
                                    {
                                        this.ReportXmiError(xmlReader, poco, "ordering", $"[{orderingValue}] is not the name of a literal of ObjectNodeOrderingKind");
                                    }
                                }
                            ...
                            case (KnowNamespacePrefixes.Uml, "parameter"):
                                CollectSingleValueReferencePropertyIdentifier(xmlReader, poco, "parameter");
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "redefinitionContext"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of ActivityParameterNode at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"ActivityParameterNodeReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: ActivityParameterNodeReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
