        ...
        public override ILoopNode Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: LoopNode [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                ...
                {
                    poco.IsTestedFirst = XmlConvert.ToBoolean(isTestedFirstXmlAttribute);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "isTestedFirst"):
                                var isTestedFirstValue = xmlReader.ReadElementContentAsStringInPlace();

                                if (!string.IsNullOrWhiteSpace(isTestedFirstValue))
                                {
                                    poco.IsTestedFirst = XmlConvert.ToBoolean(isTestedFirstValue);
                                }
                            ...
                            case (KnowNamespacePrefixes.Uml, "loopVariable"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "loopVariable", poco.LoopVariable.Count))
                                {
                            ...
                            case (KnowNamespacePrefixes.Uml, "loopVariableInput"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "loopVariableInput", poco.LoopVariableInput.Count))
                                {
