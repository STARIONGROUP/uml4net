// -------------------------------------------------------------------------------------------------
// <copyright file="ActionExecutionSpecificationReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="ActionExecutionSpecificationReader"/> is to read an instance of <see cref="IActionExecutionSpecification"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class ActionExecutionSpecificationReader : XmiElementReader<IActionExecutionSpecification>, IXmiElementReader<IActionExecutionSpecification>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<ActionExecutionSpecificationReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="ActionExecutionSpecificationReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public ActionExecutionSpecificationReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<ActionExecutionSpecificationReader>.Instance : loggerFactory.CreateLogger<ActionExecutionSpecificationReader>();
        }

        /// <summary>
        /// Reads the <see cref="IActionExecutionSpecification"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IActionExecutionSpecification"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IActionExecutionSpecification"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IActionExecutionSpecification"/>
        /// </returns>
        public override IActionExecutionSpecification Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IActionExecutionSpecification poco = new ActionExecutionSpecification();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading ActionExecutionSpecification at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.ResolveQualifiedName(xmlReader.GetXmiAttribute("type"), this.NameSpaceResolver);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:ActionExecutionSpecification")
                {
                    throw new XmlException($"The XmiType should be 'uml:ActionExecutionSpecification' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:ActionExecutionSpecification";
                }
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: ActionExecutionSpecification [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }

                var actionXmlAttribute = xmlReader.GetAttribute("action") ?? xmlReader.GetAttribute("action", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(actionXmlAttribute))
                {
                ...
                var finishXmlAttribute = xmlReader.GetAttribute("finish") ?? xmlReader.GetAttribute("finish", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(finishXmlAttribute))
                {
                ...
                var startXmlAttribute = xmlReader.GetAttribute("start") ?? xmlReader.GetAttribute("start", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(startXmlAttribute))
                {
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            case (KnowNamespacePrefixes.Uml, "action"):
                                CollectSingleValueReferencePropertyIdentifier(xmlReader, poco, "action");
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "qualifiedName"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of ActionExecutionSpecification at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"ActionExecutionSpecificationReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: ActionExecutionSpecificationReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
