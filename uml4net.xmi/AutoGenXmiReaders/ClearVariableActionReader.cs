// -------------------------------------------------------------------------------------------------
// <copyright file="ClearVariableActionReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="ClearVariableActionReader"/> is to read an instance of <see cref="IClearVariableAction"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class ClearVariableActionReader : XmiElementReader<IClearVariableAction>, IXmiElementReader<IClearVariableAction>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<ClearVariableActionReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="ClearVariableActionReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public ClearVariableActionReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<ClearVariableActionReader>.Instance : loggerFactory.CreateLogger<ClearVariableActionReader>();
        }

        /// <summary>
        /// Reads the <see cref="IClearVariableAction"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IClearVariableAction"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IClearVariableAction"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IClearVariableAction"/>
        /// </returns>
        public override IClearVariableAction Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IClearVariableAction poco = new ClearVariableAction();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading ClearVariableAction at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.GetAttribute("type", this.NameSpaceResolver.XmiNameSpace);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:ClearVariableAction")
                {
                    throw new XmlException($"The XmiType should be 'uml:ClearVariableAction' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:ClearVariableAction";
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "ClearVariableAction", poco.XmiId);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"ClearVariableActionReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: ClearVariableActionReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                                }
