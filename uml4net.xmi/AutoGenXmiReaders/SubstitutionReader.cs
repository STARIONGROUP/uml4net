...
namespace uml4net.xmi.Readers
{
    ...
    public class SubstitutionReader : XmiElementReader<ISubstitution>, IXmiElementReader<ISubstitution>
    {
        ...
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<SubstitutionReader> logger;
        ...
        /// </param>
        public SubstitutionReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<SubstitutionReader>.Instance : loggerFactory.CreateLogger<SubstitutionReader>();
        }
        ...
        public override ISubstitution Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            ...
            {
                this.logger.LogTrace("reading Substitution at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: Substitution [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                ...
                var substitutingClassifierXmlAttribute = xmlReader.GetAttribute("substitutingClassifier") ?? xmlReader.GetAttribute("substitutingClassifier", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(substitutingClassifierXmlAttribute))
                {
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "owningTemplateParameter"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "substitutingClassifier"):
                                CollectSingleValueReferencePropertyIdentifier(xmlReader, poco, "substitutingClassifier");
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "target"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of Substitution at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"SubstitutionReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: SubstitutionReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
