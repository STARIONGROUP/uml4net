// -------------------------------------------------------------------------------------------------
// <copyright file="ExtensionPointReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="ExtensionPointReader"/> is to read an instance of <see cref="IExtensionPoint"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class ExtensionPointReader : XmiElementReader<IExtensionPoint>, IXmiElementReader<IExtensionPoint>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<ExtensionPointReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="ExtensionPointReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public ExtensionPointReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<ExtensionPointReader>.Instance : loggerFactory.CreateLogger<ExtensionPointReader>();
        }

        /// <summary>
        /// Reads the <see cref="IExtensionPoint"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IExtensionPoint"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IExtensionPoint"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IExtensionPoint"/>
        /// </returns>
        public override IExtensionPoint Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IExtensionPoint poco = new ExtensionPoint();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading ExtensionPoint at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.ResolveQualifiedName(xmlReader.GetXmiAttribute("type"), this.NameSpaceResolver);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:ExtensionPoint")
                {
                    throw new XmlException($"The XmiType should be 'uml:ExtensionPoint' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:ExtensionPoint";
                }
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: ExtensionPoint [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                ...
                var useCaseXmlAttribute = xmlReader.GetAttribute("useCase") ?? xmlReader.GetAttribute("useCase", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(useCaseXmlAttribute))
                {
                    poco.SingleValueReferencePropertyIdentifiers.Add("useCase", useCaseXmlAttribute);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "redefinitionContext"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of ExtensionPoint at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"ExtensionPointReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: ExtensionPointReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
