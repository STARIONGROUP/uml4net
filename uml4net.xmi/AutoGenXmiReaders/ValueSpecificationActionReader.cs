// -------------------------------------------------------------------------------------------------
// <copyright file="ValueSpecificationActionReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="ValueSpecificationActionReader"/> is to read an instance of <see cref="IValueSpecificationAction"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class ValueSpecificationActionReader : XmiElementReader<IValueSpecificationAction>, IXmiElementReader<IValueSpecificationAction>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<ValueSpecificationActionReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="ValueSpecificationActionReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public ValueSpecificationActionReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<ValueSpecificationActionReader>.Instance : loggerFactory.CreateLogger<ValueSpecificationActionReader>();
        }

        /// <summary>
        /// Reads the <see cref="IValueSpecificationAction"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IValueSpecificationAction"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IValueSpecificationAction"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IValueSpecificationAction"/>
        /// </returns>
        public override IValueSpecificationAction Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IValueSpecificationAction poco = new ValueSpecificationAction();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading ValueSpecificationAction at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.GetAttribute("type", this.NameSpaceResolver.XmiNameSpace);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:ValueSpecificationAction")
                {
                    throw new XmlException($"The XmiType should be 'uml:ValueSpecificationAction' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:ValueSpecificationAction";
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "ValueSpecificationAction", poco.XmiId);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"ValueSpecificationActionReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: ValueSpecificationActionReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                                }
