    ...
    public class ClassifierTemplateParameterReader : XmiElementReader<IClassifierTemplateParameter>, IXmiElementReader<IClassifierTemplateParameter>
    {
        ...
        public override IClassifierTemplateParameter Read(XmlReader xmlReader)
        {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                var constrainingClassifier = new List<string>();
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmlReader.LocalName)
                        {
                            case "allowSubstitutable":
                                var allowSubstitutableXmlElement = xmlReader.ReadElementContentAsString();
                                if (!string.IsNullOrEmpty(allowSubstitutableXmlElement))
                                {
                                    poco.AllowSubstitutable = bool.Parse(allowSubstitutableXmlElement);
                                }
                                break;
                            case "constrainingClassifier":
                                this.CollectMultiValueReferencePropertyIdentifiers(xmlReader, constrainingClassifier, "constrainingClassifier");
                                break;
                            case "default":
                ...
                if (constrainingClassifier.Count > 0)
                {
                    poco.MultiValueReferencePropertyIdentifiers.Add("constrainingClassifier", constrainingClassifier);
                }
