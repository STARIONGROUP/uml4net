// -------------------------------------------------------------------------------------------------
// <copyright file="AcceptCallActionReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="AcceptCallActionReader"/> is to read an instance of <see cref="IAcceptCallAction"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class AcceptCallActionReader : XmiElementReader<IAcceptCallAction>, IXmiElementReader<IAcceptCallAction>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<AcceptCallActionReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="AcceptCallActionReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public AcceptCallActionReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<AcceptCallActionReader>.Instance : loggerFactory.CreateLogger<AcceptCallActionReader>();
        }

        /// <summary>
        /// Reads the <see cref="IAcceptCallAction"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IAcceptCallAction"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IAcceptCallAction"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IAcceptCallAction"/>
        /// </returns>
        public override IAcceptCallAction Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IAcceptCallAction poco = new AcceptCallAction();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading AcceptCallAction at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.GetAttribute("type", this.NameSpaceResolver.XmiNameSpace);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:AcceptCallAction")
                {
                    throw new XmlException($"The XmiType should be 'uml:AcceptCallAction' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:AcceptCallAction";
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "AcceptCallAction", poco.XmiId);
                }
                ...
                var isUnmarshallXmlAttribute = xmlReader.GetAttribute("isUnmarshall") ?? xmlReader.GetAttribute("isUnmarshall", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(isUnmarshallXmlAttribute))
                {
                    poco.IsUnmarshall = bool.Parse(isUnmarshallXmlAttribute);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "isLocallyReentrant"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "isUnmarshall"):
                                var isUnmarshallValue = xmlReader.ReadElementContentAsString();

                                if (!string.IsNullOrWhiteSpace(isUnmarshallValue))
                                {
                                    poco.IsUnmarshall = bool.Parse(isUnmarshallValue);
                                }
                            ...
                            case (KnowNamespacePrefixes.Uml, "result"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "returnInformation"):
                                var returnInformationValue = (IOutputPin)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.LoggerFactory, "uml:OutputPin");
                                poco.ReturnInformation.Add(returnInformationValue);
                                break;
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"AcceptCallActionReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: AcceptCallActionReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                                }
