// -------------------------------------------------------------------------------------------------
// <copyright file="ReadIsClassifiedObjectActionReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    public class ReadIsClassifiedObjectActionReader : XmiElementReader<IReadIsClassifiedObjectAction>, IXmiElementReader<IReadIsClassifiedObjectAction>
    {
        ...
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<ReadIsClassifiedObjectActionReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="ReadIsClassifiedObjectActionReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public ReadIsClassifiedObjectActionReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<ReadIsClassifiedObjectActionReader>.Instance : loggerFactory.CreateLogger<ReadIsClassifiedObjectActionReader>();
        }

        /// <summary>
        /// Reads the <see cref="IReadIsClassifiedObjectAction"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IReadIsClassifiedObjectAction"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IReadIsClassifiedObjectAction"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IReadIsClassifiedObjectAction"/>
        /// </returns>
        public override IReadIsClassifiedObjectAction Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IReadIsClassifiedObjectAction poco = new ReadIsClassifiedObjectAction();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading ReadIsClassifiedObjectAction at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.GetAttribute("type", this.NameSpaceResolver.XmiNameSpace);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:ReadIsClassifiedObjectAction")
                {
                    throw new XmlException($"The XmiType should be 'uml:ReadIsClassifiedObjectAction' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:ReadIsClassifiedObjectAction";
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "ReadIsClassifiedObjectAction", poco.XmiId);
                }
                ...
                var classifierXmlAttribute = xmlReader.GetAttribute("classifier") ?? xmlReader.GetAttribute("classifier", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(classifierXmlAttribute))
                {
                ...
                var isDirectXmlAttribute = xmlReader.GetAttribute("isDirect") ?? xmlReader.GetAttribute("isDirect", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(isDirectXmlAttribute))
                {
                    poco.IsDirect = bool.Parse(isDirectXmlAttribute);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "inStructuredNode"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "isDirect"):
                                var isDirectValue = xmlReader.ReadElementContentAsString();

                                if (!string.IsNullOrWhiteSpace(isDirectValue))
                                {
                                    poco.IsDirect = bool.Parse(isDirectValue);
                                }
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"ReadIsClassifiedObjectActionReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: ReadIsClassifiedObjectActionReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                                }
