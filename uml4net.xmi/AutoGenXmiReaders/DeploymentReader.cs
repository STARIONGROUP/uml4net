...
namespace uml4net.xmi.Readers
{
    ...
    public class DeploymentReader : XmiElementReader<IDeployment>, IXmiElementReader<IDeployment>
    {
        ...
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<DeploymentReader> logger;
        ...
        /// </param>
        public DeploymentReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<DeploymentReader>.Instance : loggerFactory.CreateLogger<DeploymentReader>();
        }
        ...
        public override IDeployment Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            ...
            {
                this.logger.LogTrace("reading Deployment at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: Deployment [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                ...
                var deployedArtifactXmlAttribute = xmlReader.GetAttribute("deployedArtifact") ?? xmlReader.GetAttribute("deployedArtifact", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(deployedArtifactXmlAttribute))
                {
                ...
                var locationXmlAttribute = xmlReader.GetAttribute("location") ?? xmlReader.GetAttribute("location", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(locationXmlAttribute))
                {
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            case (KnowNamespacePrefixes.Uml, "client"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "configuration"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "configuration", poco.Configuration.Count))
                                {
                                    var configurationValue = (IDeploymentSpecification)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.ExtenderReaderRegistry, this.LoggerFactory, "uml:DeploymentSpecification");
                                    poco.Configuration.Add(configurationValue);
                                }
                                break;
                            case (KnowNamespacePrefixes.Uml, "deployedArtifact"):
                                TryCollectMultiValueReferencePropertyIdentifiers(xmlReader, poco, "deployedArtifact");
                                break;
                            case (KnowNamespacePrefixes.Uml, "location"):
                                CollectSingleValueReferencePropertyIdentifier(xmlReader, poco, "location");
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "target"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of Deployment at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"DeploymentReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: DeploymentReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
