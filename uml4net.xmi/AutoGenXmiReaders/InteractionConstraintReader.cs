...
namespace uml4net.xmi.Readers
{
    ...
    public class InteractionConstraintReader : XmiElementReader<IInteractionConstraint>, IXmiElementReader<IInteractionConstraint>
    {
        ...
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<InteractionConstraintReader> logger;
        ...
        /// </param>
        public InteractionConstraintReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<InteractionConstraintReader>.Instance : loggerFactory.CreateLogger<InteractionConstraintReader>();
        }
        ...
        /// The name of the document that contains the <see cref="IInteractionConstraint"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IInteractionConstraint"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IInteractionConstraint"/>
        /// </returns>
        public override IInteractionConstraint Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            {
                this.logger.LogTrace("reading InteractionConstraint at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: InteractionConstraint [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "context"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "maxint"):
                            ...
                            case (KnowNamespacePrefixes.Uml, "qualifiedName"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of InteractionConstraint at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"InteractionConstraintReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: InteractionConstraintReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
