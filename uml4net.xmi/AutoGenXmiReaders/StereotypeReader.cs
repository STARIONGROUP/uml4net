// -------------------------------------------------------------------------------------------------
// <copyright file="StereotypeReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="StereotypeReader"/> is to read an instance of <see cref="IStereotype"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class StereotypeReader : XmiElementReader<IStereotype>, IXmiElementReader<IStereotype>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<StereotypeReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="StereotypeReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public StereotypeReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<StereotypeReader>.Instance : loggerFactory.CreateLogger<StereotypeReader>();
        }

        /// <summary>
        /// Reads the <see cref="IStereotype"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IStereotype"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IStereotype"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IStereotype"/>
        /// </returns>
        public override IStereotype Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IStereotype poco = new Stereotype();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading Stereotype at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.ResolveQualifiedName(xmlReader.GetXmiAttribute("type"), this.NameSpaceResolver);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:Stereotype")
                {
                    throw new XmlException($"The XmiType should be 'uml:Stereotype' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:Stereotype";
                }
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: Stereotype [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "generalization"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "icon"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "icon", poco.Icon.Count))
                                {
                                    var iconValue = (IImage)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.ExtenderReaderRegistry, this.LoggerFactory, "uml:Image");
                                    poco.Icon.Add(iconValue);
                                }
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "superClass"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of Stereotype at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"StereotypeReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: StereotypeReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
