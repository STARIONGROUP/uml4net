...
namespace uml4net.xmi.Readers.Actions
{
    ...
    public class DestroyObjectActionReader : XmiElementReader<IDestroyObjectAction>, IXmiElementReader<IDestroyObjectAction>
    {
        ...
        /// </param>
        public DestroyObjectActionReader(IXmiReaderCache cache, ILoggerFactory loggerFactory)
        ...
        /// </returns>
        public override IDestroyObjectAction Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "inStructuredNode":
                                ...
                                break;
                            case "isDestroyLinks":
                                var isDestroyLinksXmlElement = xmlReader.ReadElementContentAsString();
                                if (!string.IsNullOrEmpty(isDestroyLinksXmlElement))
                                {
                                    poco.IsDestroyLinks = bool.Parse(isDestroyLinksXmlElement);
                                }
                                break;
                            case "isDestroyOwnedObjects":
                                var isDestroyOwnedObjectsXmlElement = xmlReader.ReadElementContentAsString();
                                if (!string.IsNullOrEmpty(isDestroyOwnedObjectsXmlElement))
                                {
                                    poco.IsDestroyOwnedObjects = bool.Parse(isDestroyOwnedObjectsXmlElement);
                                }
                                break;
                            case "isLeaf":
                            ...
                            case "redefinedNode":
                                ...
                                break;
                            case "target":
                                var target = (IInputPin)this.xmiElementReaderFacade.QueryXmiElement(xmlReader, this.Cache, this.LoggerFactory, "uml:InputPin");
                                poco.Target.Add(target);
                                break;
                            case "visibility":
