// -------------------------------------------------------------------------------------------------
// <copyright file="FlowFinalNodeReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="FlowFinalNodeReader"/> is to read an instance of <see cref="IFlowFinalNode"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class FlowFinalNodeReader : XmiElementReader<IFlowFinalNode>, IXmiElementReader<IFlowFinalNode>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<FlowFinalNodeReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="FlowFinalNodeReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public FlowFinalNodeReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<FlowFinalNodeReader>.Instance : loggerFactory.CreateLogger<FlowFinalNodeReader>();
        }

        /// <summary>
        /// Reads the <see cref="IFlowFinalNode"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IFlowFinalNode"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IFlowFinalNode"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IFlowFinalNode"/>
        /// </returns>
        public override IFlowFinalNode Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IFlowFinalNode poco = new FlowFinalNode();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading FlowFinalNode at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.ResolveQualifiedName(xmlReader.GetXmiAttribute("type"), this.NameSpaceResolver);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:FlowFinalNode")
                {
                    throw new XmlException($"The XmiType should be 'uml:FlowFinalNode' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:FlowFinalNode";
                }
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: FlowFinalNode [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "redefinitionContext"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of FlowFinalNode at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"FlowFinalNodeReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: FlowFinalNodeReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
