...
namespace uml4net.xmi.Readers
{
    ...
    public class LiteralBooleanReader : XmiElementReader<ILiteralBoolean>, IXmiElementReader<ILiteralBoolean>
    {
        ...
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<LiteralBooleanReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="LiteralBooleanReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public LiteralBooleanReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<LiteralBooleanReader>.Instance : loggerFactory.CreateLogger<LiteralBooleanReader>();
        }
        ...
        /// The name of the document that contains the <see cref="ILiteralBoolean"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="ILiteralBoolean"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="ILiteralBoolean"/>
        /// </returns>
        public override ILiteralBoolean Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: LiteralBoolean [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                ...
                {
                    poco.Value = XmlConvert.ToBoolean(valueXmlAttribute);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "value"):
                                var valueValue = xmlReader.ReadElementContentAsStringInPlace();

                                if (!string.IsNullOrWhiteSpace(valueValue))
                                {
                                    poco.Value = XmlConvert.ToBoolean(valueValue);
                                }
                            ...
                            case (KnowNamespacePrefixes.Uml, "qualifiedName"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of LiteralBoolean at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"LiteralBooleanReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: LiteralBooleanReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
