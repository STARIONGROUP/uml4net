// -------------------------------------------------------------------------------------------------
// <copyright file="ReadVariableActionReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="ReadVariableActionReader"/> is to read an instance of <see cref="IReadVariableAction"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class ReadVariableActionReader : XmiElementReader<IReadVariableAction>, IXmiElementReader<IReadVariableAction>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<ReadVariableActionReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="ReadVariableActionReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public ReadVariableActionReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<ReadVariableActionReader>.Instance : loggerFactory.CreateLogger<ReadVariableActionReader>();
        }

        /// <summary>
        /// Reads the <see cref="IReadVariableAction"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IReadVariableAction"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IReadVariableAction"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IReadVariableAction"/>
        /// </returns>
        public override IReadVariableAction Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IReadVariableAction poco = new ReadVariableAction();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading ReadVariableAction at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.GetAttribute("type", this.NameSpaceResolver.XmiNameSpace);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:ReadVariableAction")
                {
                    throw new XmlException($"The XmiType should be 'uml:ReadVariableAction' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:ReadVariableAction";
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "ReadVariableAction", poco.XmiId);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"ReadVariableActionReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: ReadVariableActionReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                                }
