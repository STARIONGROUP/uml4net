...
namespace uml4net.xmi.Readers
{
    ...
    public class PrimitiveTypeReader : XmiElementReader<IPrimitiveType>, IXmiElementReader<IPrimitiveType>
    {
        ...
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<PrimitiveTypeReader> logger;
        ...
        /// </param>
        public PrimitiveTypeReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<PrimitiveTypeReader>.Instance : loggerFactory.CreateLogger<PrimitiveTypeReader>();
        }
        ...
        /// The name of the document that contains the <see cref="IPrimitiveType"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IPrimitiveType"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IPrimitiveType"/>
        /// </returns>
        public override IPrimitiveType Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            {
                this.logger.LogTrace("reading PrimitiveType at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: PrimitiveType [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "redefinitionContext"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of PrimitiveType at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"PrimitiveTypeReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: PrimitiveTypeReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
