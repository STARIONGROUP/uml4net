// -------------------------------------------------------------------------------------------------
// <copyright file="AnyReceiveEventReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="AnyReceiveEventReader"/> is to read an instance of <see cref="IAnyReceiveEvent"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class AnyReceiveEventReader : XmiElementReader<IAnyReceiveEvent>, IXmiElementReader<IAnyReceiveEvent>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<AnyReceiveEventReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="AnyReceiveEventReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public AnyReceiveEventReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<AnyReceiveEventReader>.Instance : loggerFactory.CreateLogger<AnyReceiveEventReader>();
        }

        /// <summary>
        /// Reads the <see cref="IAnyReceiveEvent"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IAnyReceiveEvent"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IAnyReceiveEvent"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IAnyReceiveEvent"/>
        /// </returns>
        public override IAnyReceiveEvent Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IAnyReceiveEvent poco = new AnyReceiveEvent();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading AnyReceiveEvent at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.ResolveQualifiedName(xmlReader.GetXmiAttribute("type"), this.NameSpaceResolver);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:AnyReceiveEvent")
                {
                    throw new XmlException($"The XmiType should be 'uml:AnyReceiveEvent' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:AnyReceiveEvent";
                }
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: AnyReceiveEvent [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "qualifiedName"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of AnyReceiveEvent at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"AnyReceiveEventReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: AnyReceiveEventReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
