// -------------------------------------------------------------------------------------------------
// <copyright file="ReplyActionReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="ReplyActionReader"/> is to read an instance of <see cref="IReplyAction"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class ReplyActionReader : XmiElementReader<IReplyAction>, IXmiElementReader<IReplyAction>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<ReplyActionReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="ReplyActionReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public ReplyActionReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<ReplyActionReader>.Instance : loggerFactory.CreateLogger<ReplyActionReader>();
        }

        /// <summary>
        /// Reads the <see cref="IReplyAction"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IReplyAction"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IReplyAction"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IReplyAction"/>
        /// </returns>
        public override IReplyAction Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IReplyAction poco = new ReplyAction();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading ReplyAction at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.ResolveQualifiedName(xmlReader.GetXmiAttribute("type"), this.NameSpaceResolver);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:ReplyAction")
                {
                    throw new XmlException($"The XmiType should be 'uml:ReplyAction' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:ReplyAction";
                }
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: ReplyAction [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                ...
                var replyToCallXmlAttribute = xmlReader.GetAttribute("replyToCall") ?? xmlReader.GetAttribute("replyToCall", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(replyToCallXmlAttribute))
                {
                    poco.SingleValueReferencePropertyIdentifiers.Add("replyToCall", replyToCallXmlAttribute);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "redefinedNode"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "replyToCall"):
                                CollectSingleValueReferencePropertyIdentifier(xmlReader, poco, "replyToCall");
                                break;
                            case (KnowNamespacePrefixes.Uml, "replyValue"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "replyValue", poco.ReplyValue.Count))
                                {
                                    var replyValueValue = (IInputPin)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.ExtenderReaderRegistry, this.LoggerFactory, "uml:InputPin");
                                    poco.ReplyValue.Add(replyValueValue);
                                }
                                break;
                            case (KnowNamespacePrefixes.Uml, "returnInformation"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "returnInformation", poco.ReturnInformation.Count))
                                {
                                    var returnInformationValue = (IInputPin)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.ExtenderReaderRegistry, this.LoggerFactory, "uml:InputPin");
                            ...
                            case (KnowNamespacePrefixes.Uml, "redefinitionContext"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of ReplyAction at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"ReplyActionReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: ReplyActionReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
