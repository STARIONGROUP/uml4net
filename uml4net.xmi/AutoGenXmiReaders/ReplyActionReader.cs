...
namespace uml4net.xmi.Readers
{
    ...
    public class ReplyActionReader : XmiElementReader<IReplyAction>, IXmiElementReader<IReplyAction>
    {
        ...
        /// </param>
        public ReplyActionReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, ILoggerFactory loggerFactory)
        ...
        public override IReplyAction Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "ReplyAction", poco.XmiId);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "replyToCall"):
                                this.CollectSingleValueReferencePropertyIdentifier(xmlReader, poco, "replyToCall");
                                break;
                            case (KnowNamespacePrefixes.Uml, "replyValue"):
                                var replyValueValue = (IInputPin)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.LoggerFactory, "uml:InputPin");
                                poco.ReplyValue.Add(replyValueValue);
                                break;
                            case (KnowNamespacePrefixes.Uml, "returnInformation"):
                                var returnInformationValue = (IInputPin)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.LoggerFactory, "uml:InputPin");
