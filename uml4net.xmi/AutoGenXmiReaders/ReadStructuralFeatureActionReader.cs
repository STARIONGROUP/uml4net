// -------------------------------------------------------------------------------------------------
// <copyright file="ReadStructuralFeatureActionReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    public class ReadStructuralFeatureActionReader : XmiElementReader<IReadStructuralFeatureAction>, IXmiElementReader<IReadStructuralFeatureAction>
    {
        ...
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<ReadStructuralFeatureActionReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="ReadStructuralFeatureActionReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public ReadStructuralFeatureActionReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<ReadStructuralFeatureActionReader>.Instance : loggerFactory.CreateLogger<ReadStructuralFeatureActionReader>();
        }

        /// <summary>
        /// Reads the <see cref="IReadStructuralFeatureAction"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IReadStructuralFeatureAction"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IReadStructuralFeatureAction"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IReadStructuralFeatureAction"/>
        /// </returns>
        public override IReadStructuralFeatureAction Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IReadStructuralFeatureAction poco = new ReadStructuralFeatureAction();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading ReadStructuralFeatureAction at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.GetAttribute("type", this.NameSpaceResolver.XmiNameSpace);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:ReadStructuralFeatureAction")
                {
                    throw new XmlException($"The XmiType should be 'uml:ReadStructuralFeatureAction' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:ReadStructuralFeatureAction";
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "ReadStructuralFeatureAction", poco.XmiId);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"ReadStructuralFeatureActionReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: ReadStructuralFeatureActionReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                                }
