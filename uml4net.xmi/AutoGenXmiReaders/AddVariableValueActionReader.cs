// -------------------------------------------------------------------------------------------------
// <copyright file="AddVariableValueActionReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="AddVariableValueActionReader"/> is to read an instance of <see cref="IAddVariableValueAction"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class AddVariableValueActionReader : XmiElementReader<IAddVariableValueAction>, IXmiElementReader<IAddVariableValueAction>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<AddVariableValueActionReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="AddVariableValueActionReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public AddVariableValueActionReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<AddVariableValueActionReader>.Instance : loggerFactory.CreateLogger<AddVariableValueActionReader>();
        }

        /// <summary>
        /// Reads the <see cref="IAddVariableValueAction"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IAddVariableValueAction"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IAddVariableValueAction"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IAddVariableValueAction"/>
        /// </returns>
        public override IAddVariableValueAction Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IAddVariableValueAction poco = new AddVariableValueAction();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading AddVariableValueAction at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.GetAttribute("type", this.NameSpaceResolver.XmiNameSpace);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:AddVariableValueAction")
                {
                    throw new XmlException($"The XmiType should be 'uml:AddVariableValueAction' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:AddVariableValueAction";
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "AddVariableValueAction", poco.XmiId);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "inPartition"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "insertAt"):
                                var insertAtValue = (IInputPin)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.LoggerFactory, "uml:InputPin");
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"AddVariableValueActionReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: AddVariableValueActionReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                                }
