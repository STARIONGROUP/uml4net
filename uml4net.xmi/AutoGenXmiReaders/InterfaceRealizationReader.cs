...
namespace uml4net.xmi.Readers
{
    ...
    public class InterfaceRealizationReader : XmiElementReader<IInterfaceRealization>, IXmiElementReader<IInterfaceRealization>
    {
        ...
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<InterfaceRealizationReader> logger;
        ...
        /// </param>
        public InterfaceRealizationReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<InterfaceRealizationReader>.Instance : loggerFactory.CreateLogger<InterfaceRealizationReader>();
        }
        ...
        /// The name of the document that contains the <see cref="IInterfaceRealization"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IInterfaceRealization"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IInterfaceRealization"/>
        /// </returns>
        public override IInterfaceRealization Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            {
                this.logger.LogTrace("reading InterfaceRealization at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: InterfaceRealization [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                ...
                var contractXmlAttribute = xmlReader.GetAttribute("contract") ?? xmlReader.GetAttribute("contract", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(contractXmlAttribute))
                {
                    poco.SingleValueReferencePropertyIdentifiers.Add("contract", contractXmlAttribute);
                }

                var implementingClassifierXmlAttribute = xmlReader.GetAttribute("implementingClassifier") ?? xmlReader.GetAttribute("implementingClassifier", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(implementingClassifierXmlAttribute))
                {
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "contract"):
                                CollectSingleValueReferencePropertyIdentifier(xmlReader, poco, "contract");
                                break;
                            case (KnowNamespacePrefixes.Uml, "implementingClassifier"):
                                CollectSingleValueReferencePropertyIdentifier(xmlReader, poco, "implementingClassifier");
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "target"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of InterfaceRealization at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"InterfaceRealizationReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: InterfaceRealizationReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
