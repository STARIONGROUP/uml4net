// -------------------------------------------------------------------------------------------------
// <copyright file="TimeIntervalReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="TimeIntervalReader"/> is to read an instance of <see cref="ITimeInterval"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class TimeIntervalReader : XmiElementReader<ITimeInterval>, IXmiElementReader<ITimeInterval>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<TimeIntervalReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="TimeIntervalReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public TimeIntervalReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<TimeIntervalReader>.Instance : loggerFactory.CreateLogger<TimeIntervalReader>();
        }

        /// <summary>
        /// Reads the <see cref="ITimeInterval"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="ITimeInterval"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="ITimeInterval"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="ITimeInterval"/>
        /// </returns>
        public override ITimeInterval Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            ITimeInterval poco = new TimeInterval();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading TimeInterval at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.ResolveQualifiedName(xmlReader.GetXmiAttribute("type"), this.NameSpaceResolver);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:TimeInterval")
                {
                    throw new XmlException($"The XmiType should be 'uml:TimeInterval' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:TimeInterval";
                }
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: TimeInterval [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }

                var maxXmlAttribute = xmlReader.GetAttribute("max") ?? xmlReader.GetAttribute("max", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(maxXmlAttribute))
                {
                    poco.SingleValueReferencePropertyIdentifiers.Add("max", maxXmlAttribute);
                }

                var minXmlAttribute = xmlReader.GetAttribute("min") ?? xmlReader.GetAttribute("min", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(minXmlAttribute))
                {
                ...
                while (xmlReader.Read())
                {
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        {
                            case (KnowNamespacePrefixes.Uml, "max"):
                                CollectSingleValueReferencePropertyIdentifier(xmlReader, poco, "max");
                                break;
                            case (KnowNamespacePrefixes.Uml, "min"):
                                CollectSingleValueReferencePropertyIdentifier(xmlReader, poco, "min");
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "qualifiedName"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of TimeInterval at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"TimeIntervalReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: TimeIntervalReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
