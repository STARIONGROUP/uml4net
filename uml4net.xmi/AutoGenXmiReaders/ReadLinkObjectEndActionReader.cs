// -------------------------------------------------------------------------------------------------
// <copyright file="ReadLinkObjectEndActionReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="ReadLinkObjectEndActionReader"/> is to read an instance of <see cref="IReadLinkObjectEndAction"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class ReadLinkObjectEndActionReader : XmiElementReader<IReadLinkObjectEndAction>, IXmiElementReader<IReadLinkObjectEndAction>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<ReadLinkObjectEndActionReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="ReadLinkObjectEndActionReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public ReadLinkObjectEndActionReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<ReadLinkObjectEndActionReader>.Instance : loggerFactory.CreateLogger<ReadLinkObjectEndActionReader>();
        }

        /// <summary>
        /// Reads the <see cref="IReadLinkObjectEndAction"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IReadLinkObjectEndAction"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IReadLinkObjectEndAction"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IReadLinkObjectEndAction"/>
        /// </returns>
        public override IReadLinkObjectEndAction Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IReadLinkObjectEndAction poco = new ReadLinkObjectEndAction();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading ReadLinkObjectEndAction at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.GetAttribute("type", this.NameSpaceResolver.XmiNameSpace);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:ReadLinkObjectEndAction")
                {
                    throw new XmlException($"The XmiType should be 'uml:ReadLinkObjectEndAction' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:ReadLinkObjectEndAction";
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "ReadLinkObjectEndAction", poco.XmiId);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"ReadLinkObjectEndActionReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: ReadLinkObjectEndActionReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                                }
