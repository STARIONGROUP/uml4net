...
namespace uml4net.xmi.Readers
{
    ...
    public class PseudostateReader : XmiElementReader<IPseudostate>, IXmiElementReader<IPseudostate>
    {
        ...
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<PseudostateReader> logger;
        ...
        /// </param>
        public PseudostateReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<PseudostateReader>.Instance : loggerFactory.CreateLogger<PseudostateReader>();
        }
        ...
        /// The name of the document that contains the <see cref="IPseudostate"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IPseudostate"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IPseudostate"/>
        /// </returns>
        public override IPseudostate Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            {
                this.logger.LogTrace("reading Pseudostate at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: Pseudostate [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                ...
                {
                    if (PseudostateKindExtensions.TryParseXmiLiteral(kindXmlAttribute, out var kindLiteral))
                    {
                        poco.Kind = kindLiteral;
                    }
                    else
                    {
                        this.ReportXmiError(xmlReader, poco, "kind", $"[{kindXmlAttribute}] is not the name of a literal of PseudostateKind");
                    }
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "kind"):
                                ...
                                {
                                    if (PseudostateKindExtensions.TryParseXmiLiteral(kindValue, out var kindLiteral))
                                    {
                                        poco.Kind = kindLiteral;
                                    }
                                    else
                                    {
                                        this.ReportXmiError(xmlReader, poco, "kind", $"[{kindValue}] is not the name of a literal of PseudostateKind");
                                    }
                                }
                            ...
                            case (KnowNamespacePrefixes.Uml, "redefinitionContext"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of Pseudostate at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"PseudostateReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: PseudostateReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
