// -------------------------------------------------------------------------------------------------
// <copyright file="CallOperationActionReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="CallOperationActionReader"/> is to read an instance of <see cref="ICallOperationAction"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class CallOperationActionReader : XmiElementReader<ICallOperationAction>, IXmiElementReader<ICallOperationAction>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<CallOperationActionReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="CallOperationActionReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public CallOperationActionReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<CallOperationActionReader>.Instance : loggerFactory.CreateLogger<CallOperationActionReader>();
        }

        /// <summary>
        /// Reads the <see cref="ICallOperationAction"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="ICallOperationAction"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="ICallOperationAction"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="ICallOperationAction"/>
        /// </returns>
        public override ICallOperationAction Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            ICallOperationAction poco = new CallOperationAction();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading CallOperationAction at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.GetAttribute("type", this.NameSpaceResolver.XmiNameSpace);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:CallOperationAction")
                {
                    throw new XmlException($"The XmiType should be 'uml:CallOperationAction' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:CallOperationAction";
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "CallOperationAction", poco.XmiId);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "onPort"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "operation"):
                            ...
                            case (KnowNamespacePrefixes.Uml, "target"):
                                var targetValue = (IInputPin)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.LoggerFactory, "uml:InputPin");
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"CallOperationActionReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: CallOperationActionReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                                }
