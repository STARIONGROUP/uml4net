...
namespace uml4net.xmi.Readers
{
    ...
    public class AssociationClassReader : XmiElementReader<IAssociationClass>, IXmiElementReader<IAssociationClass>
    {
        ...
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<AssociationClassReader> logger;
        ...
        /// </param>
        public AssociationClassReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<AssociationClassReader>.Instance : loggerFactory.CreateLogger<AssociationClassReader>();
        }
        ...
        /// The name of the document that contains the <see cref="IAssociationClass"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IAssociationClass"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IAssociationClass"/>
        /// </returns>
        public override IAssociationClass Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            {
                this.logger.LogTrace("reading AssociationClass at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: AssociationClass [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "isDerived"):
                                var isDerivedValue = xmlReader.ReadElementContentAsStringInPlace();
                            ...
                            case (KnowNamespacePrefixes.Uml, "ownedEnd"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "ownedEnd", poco.OwnedEnd.Count))
                                {
                            ...
                            case (KnowNamespacePrefixes.Uml, "endType"):
                            ...
                            case (KnowNamespacePrefixes.Uml, "superClass"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of AssociationClass at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"AssociationClassReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: AssociationClassReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
