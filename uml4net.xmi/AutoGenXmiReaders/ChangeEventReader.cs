...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="ChangeEventReader"/> is to read an instance of <see cref="IChangeEvent"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class ChangeEventReader : XmiElementReader<IChangeEvent>, IXmiElementReader<IChangeEvent>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<ChangeEventReader> logger;
        ...
        /// </param>
        public ChangeEventReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<ChangeEventReader>.Instance : loggerFactory.CreateLogger<ChangeEventReader>();
        }

        /// <summary>
        /// Reads the <see cref="IChangeEvent"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IChangeEvent"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IChangeEvent"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IChangeEvent"/>
        /// </returns>
        public override IChangeEvent Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IChangeEvent poco = new ChangeEvent();
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:ChangeEvent")
                {
                    throw new XmlException($"The XmiType should be 'uml:ChangeEvent' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:ChangeEvent";
                }
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: ChangeEvent [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                ...
                while (xmlReader.Read())
                {
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        {
                            case (KnowNamespacePrefixes.Uml, "changeExpression"):
                            ...
                            case (KnowNamespacePrefixes.Uml, "qualifiedName"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of ChangeEvent at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    this.logger.LogWarning("Not Supported: ChangeEventReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
