// -------------------------------------------------------------------------------------------------
// <copyright file="ReadLinkActionReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="ReadLinkActionReader"/> is to read an instance of <see cref="IReadLinkAction"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class ReadLinkActionReader : XmiElementReader<IReadLinkAction>, IXmiElementReader<IReadLinkAction>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<ReadLinkActionReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="ReadLinkActionReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public ReadLinkActionReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<ReadLinkActionReader>.Instance : loggerFactory.CreateLogger<ReadLinkActionReader>();
        }

        /// <summary>
        /// Reads the <see cref="IReadLinkAction"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IReadLinkAction"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IReadLinkAction"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IReadLinkAction"/>
        /// </returns>
        public override IReadLinkAction Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IReadLinkAction poco = new ReadLinkAction();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading ReadLinkAction at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.GetAttribute("type", this.NameSpaceResolver.XmiNameSpace);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:ReadLinkAction")
                {
                    throw new XmlException($"The XmiType should be 'uml:ReadLinkAction' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:ReadLinkAction";
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "ReadLinkAction", poco.XmiId);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "endData"):
                                var endDataValue = (ILinkEndData)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.LoggerFactory, "uml:LinkEndData");
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"ReadLinkActionReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: ReadLinkActionReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                                }
