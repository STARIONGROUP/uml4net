// -------------------------------------------------------------------------------------------------
// <copyright file="ContinuationReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="ContinuationReader"/> is to read an instance of <see cref="IContinuation"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class ContinuationReader : XmiElementReader<IContinuation>, IXmiElementReader<IContinuation>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<ContinuationReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="ContinuationReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public ContinuationReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<ContinuationReader>.Instance : loggerFactory.CreateLogger<ContinuationReader>();
        }

        /// <summary>
        /// Reads the <see cref="IContinuation"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IContinuation"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IContinuation"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IContinuation"/>
        /// </returns>
        public override IContinuation Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IContinuation poco = new Continuation();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading Continuation at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.ResolveQualifiedName(xmlReader.GetXmiAttribute("type"), this.NameSpaceResolver);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:Continuation")
                {
                    throw new XmlException($"The XmiType should be 'uml:Continuation' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:Continuation";
                }
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: Continuation [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                ...
                var settingXmlAttribute = xmlReader.GetAttribute("setting") ?? xmlReader.GetAttribute("setting", this.NameSpaceResolver.UmlNameSpace);
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "ownedComment"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "setting"):
                                var settingValue = xmlReader.ReadElementContentAsStringInPlace();
                            ...
                            case (KnowNamespacePrefixes.Uml, "qualifiedName"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of Continuation at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"ContinuationReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: ContinuationReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
