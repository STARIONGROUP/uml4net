...
namespace uml4net.xmi.Readers
{
    ...
    public class InformationFlowReader : XmiElementReader<IInformationFlow>, IXmiElementReader<IInformationFlow>
    {
        ...
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<InformationFlowReader> logger;
        ...
        /// </param>
        public InformationFlowReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<InformationFlowReader>.Instance : loggerFactory.CreateLogger<InformationFlowReader>();
        }
        ...
        public override IInformationFlow Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            ...
            {
                this.logger.LogTrace("reading InformationFlow at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: InformationFlow [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }

                var conveyedXmlAttribute = xmlReader.GetAttribute("conveyed") ?? xmlReader.GetAttribute("conveyed", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(conveyedXmlAttribute))
                {
                ...
                var informationSourceXmlAttribute = xmlReader.GetAttribute("informationSource") ?? xmlReader.GetAttribute("informationSource", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(informationSourceXmlAttribute))
                {
                ...
                var informationTargetXmlAttribute = xmlReader.GetAttribute("informationTarget") ?? xmlReader.GetAttribute("informationTarget", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(informationTargetXmlAttribute))
                {
                ...
                var realizationXmlAttribute = xmlReader.GetAttribute("realization") ?? xmlReader.GetAttribute("realization", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(realizationXmlAttribute))
                {
                ...
                var realizingActivityEdgeXmlAttribute = xmlReader.GetAttribute("realizingActivityEdge") ?? xmlReader.GetAttribute("realizingActivityEdge", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(realizingActivityEdgeXmlAttribute))
                {
                ...
                var realizingConnectorXmlAttribute = xmlReader.GetAttribute("realizingConnector") ?? xmlReader.GetAttribute("realizingConnector", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(realizingConnectorXmlAttribute))
                {
                ...
                var realizingMessageXmlAttribute = xmlReader.GetAttribute("realizingMessage") ?? xmlReader.GetAttribute("realizingMessage", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(realizingMessageXmlAttribute))
                {
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "target"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of InformationFlow at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"InformationFlowReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: InformationFlowReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
