// -------------------------------------------------------------------------------------------------
// <copyright file="InteractionReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    public class InteractionReader : XmiElementReader<IInteraction>, IXmiElementReader<IInteraction>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<InteractionReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="InteractionReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public InteractionReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<InteractionReader>.Instance : loggerFactory.CreateLogger<InteractionReader>();
        }

        /// <summary>
        /// Reads the <see cref="IInteraction"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IInteraction"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IInteraction"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IInteraction"/>
        /// </returns>
        public override IInteraction Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            {
                this.logger.LogTrace("reading Interaction at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.ResolveQualifiedName(xmlReader.GetXmiAttribute("type"), this.NameSpaceResolver);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:Interaction")
                {
                    throw new XmlException($"The XmiType should be 'uml:Interaction' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:Interaction";
                }
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: Interaction [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                ...
                while (xmlReader.Read())
                {
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        {
                            case (KnowNamespacePrefixes.Uml, "action"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "action", poco.Action.Count))
                                {
                                    var actionValue = (IAction)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.ExtenderReaderRegistry, this.LoggerFactory);
                            ...
                            case (KnowNamespacePrefixes.Uml, "enclosingOperand"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "formalGate"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "formalGate", poco.FormalGate.Count))
                                {
                                    var formalGateValue = (IGate)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.ExtenderReaderRegistry, this.LoggerFactory, "uml:Gate");
                                    poco.FormalGate.Add(formalGateValue);
                                }
                                break;
                            case (KnowNamespacePrefixes.Uml, "fragment"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "fragment", poco.Fragment.Count))
                                {
                                    var fragmentValue = (IInteractionFragment)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.ExtenderReaderRegistry, this.LoggerFactory);
                            ...
                            case (KnowNamespacePrefixes.Uml, "isReentrant"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "lifeline"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "lifeline", poco.Lifeline.Count))
                                {
                                    var lifelineValue = (ILifeline)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.ExtenderReaderRegistry, this.LoggerFactory, "uml:Lifeline");
                                    poco.Lifeline.Add(lifelineValue);
                                }
                                break;
                            case (KnowNamespacePrefixes.Uml, "message"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "message", poco.Message.Count))
                                {
                                    var messageValue = (IMessage)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.ExtenderReaderRegistry, this.LoggerFactory, "uml:Message");
                            ...
                            case (KnowNamespacePrefixes.Uml, "superClass"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of Interaction at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"InteractionReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: InteractionReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
