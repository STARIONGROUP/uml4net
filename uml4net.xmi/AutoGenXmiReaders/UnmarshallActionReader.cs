// -------------------------------------------------------------------------------------------------
// <copyright file="UnmarshallActionReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="UnmarshallActionReader"/> is to read an instance of <see cref="IUnmarshallAction"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class UnmarshallActionReader : XmiElementReader<IUnmarshallAction>, IXmiElementReader<IUnmarshallAction>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<UnmarshallActionReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="UnmarshallActionReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public UnmarshallActionReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<UnmarshallActionReader>.Instance : loggerFactory.CreateLogger<UnmarshallActionReader>();
        }

        /// <summary>
        /// Reads the <see cref="IUnmarshallAction"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IUnmarshallAction"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IUnmarshallAction"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IUnmarshallAction"/>
        /// </returns>
        public override IUnmarshallAction Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IUnmarshallAction poco = new UnmarshallAction();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading UnmarshallAction at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.GetAttribute("type", this.NameSpaceResolver.XmiNameSpace);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:UnmarshallAction")
                {
                    throw new XmlException($"The XmiType should be 'uml:UnmarshallAction' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:UnmarshallAction";
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "UnmarshallAction", poco.XmiId);
                }
                ...
                var unmarshallTypeXmlAttribute = xmlReader.GetAttribute("unmarshallType") ?? xmlReader.GetAttribute("unmarshallType", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(unmarshallTypeXmlAttribute))
                {
                    poco.SingleValueReferencePropertyIdentifiers.Add("unmarshallType", unmarshallTypeXmlAttribute);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "result"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "unmarshallType"):
                                this.CollectSingleValueReferencePropertyIdentifier(xmlReader, poco, "unmarshallType");
                                break;
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"UnmarshallActionReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: UnmarshallActionReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                                }
