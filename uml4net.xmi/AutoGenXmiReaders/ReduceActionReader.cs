// -------------------------------------------------------------------------------------------------
// <copyright file="ReduceActionReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    public class ReduceActionReader : XmiElementReader<IReduceAction>, IXmiElementReader<IReduceAction>
    {
        ...
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<ReduceActionReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="ReduceActionReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public ReduceActionReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<ReduceActionReader>.Instance : loggerFactory.CreateLogger<ReduceActionReader>();
        }

        /// <summary>
        /// Reads the <see cref="IReduceAction"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IReduceAction"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IReduceAction"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IReduceAction"/>
        /// </returns>
        public override IReduceAction Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IReduceAction poco = new ReduceAction();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading ReduceAction at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.GetAttribute("type", this.NameSpaceResolver.XmiNameSpace);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:ReduceAction")
                {
                    throw new XmlException($"The XmiType should be 'uml:ReduceAction' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:ReduceAction";
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "ReduceAction", poco.XmiId);
                }
                ...
                var reducerXmlAttribute = xmlReader.GetAttribute("reducer") ?? xmlReader.GetAttribute("reducer", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(reducerXmlAttribute))
                {
                    poco.SingleValueReferencePropertyIdentifiers.Add("reducer", reducerXmlAttribute);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            case (KnowNamespacePrefixes.Uml, "activity"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "collection"):
                                var collectionValue = (IInputPin)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.LoggerFactory, "uml:InputPin");
                                poco.Collection.Add(collectionValue);
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "redefinedNode"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "reducer"):
                                this.CollectSingleValueReferencePropertyIdentifier(xmlReader, poco, "reducer");
                                break;
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"ReduceActionReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: ReduceActionReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                                }
