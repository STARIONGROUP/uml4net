// -------------------------------------------------------------------------------------------------
// <copyright file="PartDecompositionReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="PartDecompositionReader"/> is to read an instance of <see cref="IPartDecomposition"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class PartDecompositionReader : XmiElementReader<IPartDecomposition>, IXmiElementReader<IPartDecomposition>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<PartDecompositionReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="PartDecompositionReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public PartDecompositionReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<PartDecompositionReader>.Instance : loggerFactory.CreateLogger<PartDecompositionReader>();
        }

        /// <summary>
        /// Reads the <see cref="IPartDecomposition"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IPartDecomposition"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IPartDecomposition"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IPartDecomposition"/>
        /// </returns>
        public override IPartDecomposition Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IPartDecomposition poco = new PartDecomposition();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading PartDecomposition at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.ResolveQualifiedName(xmlReader.GetXmiAttribute("type"), this.NameSpaceResolver);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:PartDecomposition")
                {
                    throw new XmlException($"The XmiType should be 'uml:PartDecomposition' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:PartDecomposition";
                }
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: PartDecomposition [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                ...
                var refersToXmlAttribute = xmlReader.GetAttribute("refersTo") ?? xmlReader.GetAttribute("refersTo", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(refersToXmlAttribute))
                {
                    poco.SingleValueReferencePropertyIdentifiers.Add("refersTo", refersToXmlAttribute);
                }

                var returnValueRecipientXmlAttribute = xmlReader.GetAttribute("returnValueRecipient") ?? xmlReader.GetAttribute("returnValueRecipient", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(returnValueRecipientXmlAttribute))
                {
                    poco.SingleValueReferencePropertyIdentifiers.Add("returnValueRecipient", returnValueRecipientXmlAttribute);
                }
                ...
                while (xmlReader.Read())
                {
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        {
                            case (KnowNamespacePrefixes.Uml, "actualGate"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "actualGate", poco.ActualGate.Count))
                                {
                                    var actualGateValue = (IGate)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.ExtenderReaderRegistry, this.LoggerFactory, "uml:Gate");
                                    poco.ActualGate.Add(actualGateValue);
                                }
                                break;
                            case (KnowNamespacePrefixes.Uml, "argument"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "argument", poco.Argument.Count))
                                {
                            ...
                            case (KnowNamespacePrefixes.Uml, "ownedComment"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "refersTo"):
                                CollectSingleValueReferencePropertyIdentifier(xmlReader, poco, "refersTo");
                                break;
                            case (KnowNamespacePrefixes.Uml, "returnValue"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "returnValue", poco.ReturnValue.Count))
                                {
                                    var returnValueValue = (IValueSpecification)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.ExtenderReaderRegistry, this.LoggerFactory);
                                    poco.ReturnValue.Add(returnValueValue);
                                }
                                break;
                            case (KnowNamespacePrefixes.Uml, "returnValueRecipient"):
                                CollectSingleValueReferencePropertyIdentifier(xmlReader, poco, "returnValueRecipient");
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "qualifiedName"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of PartDecomposition at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"PartDecompositionReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: PartDecompositionReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
