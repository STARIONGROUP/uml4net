...
namespace uml4net.xmi.Readers
{
    ...
    public class ExpansionNodeReader : XmiElementReader<IExpansionNode>, IXmiElementReader<IExpansionNode>
    {
        ...
        /// </param>
        public ExpansionNodeReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
        ...
        public override IExpansionNode Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: ExpansionNode [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                ...
                var regionAsInputXmlAttribute = xmlReader.GetAttribute("regionAsInput") ?? xmlReader.GetAttribute("regionAsInput", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(regionAsInputXmlAttribute))
                {
                    poco.SingleValueReferencePropertyIdentifiers.Add("regionAsInput", regionAsInputXmlAttribute);
                }

                var regionAsOutputXmlAttribute = xmlReader.GetAttribute("regionAsOutput") ?? xmlReader.GetAttribute("regionAsOutput", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(regionAsOutputXmlAttribute))
                {
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "redefinedNode"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "regionAsInput"):
                                CollectSingleValueReferencePropertyIdentifier(xmlReader, poco, "regionAsInput");
                                break;
                            case (KnowNamespacePrefixes.Uml, "regionAsOutput"):
                                CollectSingleValueReferencePropertyIdentifier(xmlReader, poco, "regionAsOutput");
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "redefinitionContext"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of ExpansionNode at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
