...
namespace uml4net.xmi.Readers
{
    ...
    public class ProtocolStateMachineReader : XmiElementReader<IProtocolStateMachine>, IXmiElementReader<IProtocolStateMachine>
    {
        ...
        /// </param>
        public ProtocolStateMachineReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
        ...
        public override IProtocolStateMachine Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: ProtocolStateMachine [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "conformance"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "conformance", poco.Conformance.Count))
                                {
                                    var conformanceValue = (IProtocolConformance)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.ExtenderReaderRegistry, this.LoggerFactory, "uml:ProtocolConformance");
                            ...
                            case (KnowNamespacePrefixes.Uml, "superClass"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of ProtocolStateMachine at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
