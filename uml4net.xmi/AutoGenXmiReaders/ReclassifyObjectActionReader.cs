// -------------------------------------------------------------------------------------------------
// <copyright file="ReclassifyObjectActionReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="ReclassifyObjectActionReader"/> is to read an instance of <see cref="IReclassifyObjectAction"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class ReclassifyObjectActionReader : XmiElementReader<IReclassifyObjectAction>, IXmiElementReader<IReclassifyObjectAction>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<ReclassifyObjectActionReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="ReclassifyObjectActionReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public ReclassifyObjectActionReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<ReclassifyObjectActionReader>.Instance : loggerFactory.CreateLogger<ReclassifyObjectActionReader>();
        }

        /// <summary>
        /// Reads the <see cref="IReclassifyObjectAction"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IReclassifyObjectAction"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IReclassifyObjectAction"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IReclassifyObjectAction"/>
        /// </returns>
        public override IReclassifyObjectAction Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IReclassifyObjectAction poco = new ReclassifyObjectAction();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading ReclassifyObjectAction at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.GetAttribute("type", this.NameSpaceResolver.XmiNameSpace);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:ReclassifyObjectAction")
                {
                    throw new XmlException($"The XmiType should be 'uml:ReclassifyObjectAction' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:ReclassifyObjectAction";
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "ReclassifyObjectAction", poco.XmiId);
                }
                ...
                var newClassifierXmlAttribute = xmlReader.GetAttribute("newClassifier") ?? xmlReader.GetAttribute("newClassifier", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(newClassifierXmlAttribute))
                {
                    var newClassifierXmlAttributeValues = newClassifierXmlAttribute.Split(SplitMultiReference, StringSplitOptions.RemoveEmptyEntries).ToList();
                    poco.MultiValueReferencePropertyIdentifiers.Add("newClassifier", newClassifierXmlAttributeValues);
                }

                var oldClassifierXmlAttribute = xmlReader.GetAttribute("oldClassifier") ?? xmlReader.GetAttribute("oldClassifier", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(oldClassifierXmlAttribute))
                {
                    var oldClassifierXmlAttributeValues = oldClassifierXmlAttribute.Split(SplitMultiReference, StringSplitOptions.RemoveEmptyEntries).ToList();
                    poco.MultiValueReferencePropertyIdentifiers.Add("oldClassifier", oldClassifierXmlAttributeValues);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "nameExpression"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "newClassifier"):
                                this.TryCollectMultiValueReferencePropertyIdentifiers(xmlReader, poco, "newClassifier");
                                break;
                            case (KnowNamespacePrefixes.Uml, "object"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "oldClassifier"):
                                this.TryCollectMultiValueReferencePropertyIdentifiers(xmlReader, poco, "oldClassifier");
                                break;
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"ReclassifyObjectActionReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: ReclassifyObjectActionReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                                }
