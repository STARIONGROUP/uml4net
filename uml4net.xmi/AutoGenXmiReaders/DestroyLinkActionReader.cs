// -------------------------------------------------------------------------------------------------
// <copyright file="DestroyLinkActionReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    public class DestroyLinkActionReader : XmiElementReader<IDestroyLinkAction>, IXmiElementReader<IDestroyLinkAction>
    {
        ...
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<DestroyLinkActionReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="DestroyLinkActionReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public DestroyLinkActionReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<DestroyLinkActionReader>.Instance : loggerFactory.CreateLogger<DestroyLinkActionReader>();
        }

        /// <summary>
        /// Reads the <see cref="IDestroyLinkAction"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IDestroyLinkAction"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IDestroyLinkAction"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IDestroyLinkAction"/>
        /// </returns>
        public override IDestroyLinkAction Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IDestroyLinkAction poco = new DestroyLinkAction();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading DestroyLinkAction at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.ResolveQualifiedName(xmlReader.GetXmiAttribute("type"), this.NameSpaceResolver);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:DestroyLinkAction")
                {
                    throw new XmlException($"The XmiType should be 'uml:DestroyLinkAction' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:DestroyLinkAction";
                }
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: DestroyLinkAction [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "endData"):
                                ...
                                {
                                    var endDataValue = (ILinkEndDestructionData)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.ExtenderReaderRegistry, this.LoggerFactory, "uml:LinkEndDestructionData");
                            ...
                            case (KnowNamespacePrefixes.Uml, "redefinitionContext"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of DestroyLinkAction at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"DestroyLinkActionReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: DestroyLinkActionReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
