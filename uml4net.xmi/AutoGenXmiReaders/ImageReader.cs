...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="ImageReader"/> is to read an instance of <see cref="IImage"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class ImageReader : XmiElementReader<IImage>, IXmiElementReader<IImage>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<ImageReader> logger;
        ...
        /// </param>
        public ImageReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<ImageReader>.Instance : loggerFactory.CreateLogger<ImageReader>();
        }

        /// <summary>
        /// Reads the <see cref="IImage"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IImage"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IImage"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IImage"/>
        /// </returns>
        public override IImage Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:Image")
                {
                    throw new XmlException($"The XmiType should be 'uml:Image' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:Image";
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: Image [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }

                poco.Content = xmlReader.GetAttribute("content") ?? xmlReader.GetAttribute("content", this.NameSpaceResolver.UmlNameSpace);

                poco.Format = xmlReader.GetAttribute("format") ?? xmlReader.GetAttribute("format", this.NameSpaceResolver.UmlNameSpace);

                poco.Location = xmlReader.GetAttribute("location") ?? xmlReader.GetAttribute("location", this.NameSpaceResolver.UmlNameSpace);
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "owner"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of Image at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    this.logger.LogWarning("Not Supported: ImageReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
