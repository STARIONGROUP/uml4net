// -------------------------------------------------------------------------------------------------
// <copyright file="InformationItemReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="InformationItemReader"/> is to read an instance of <see cref="IInformationItem"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class InformationItemReader : XmiElementReader<IInformationItem>, IXmiElementReader<IInformationItem>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<InformationItemReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="InformationItemReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public InformationItemReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<InformationItemReader>.Instance : loggerFactory.CreateLogger<InformationItemReader>();
        }

        /// <summary>
        /// Reads the <see cref="IInformationItem"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IInformationItem"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IInformationItem"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IInformationItem"/>
        /// </returns>
        public override IInformationItem Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IInformationItem poco = new InformationItem();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading InformationItem at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.ResolveQualifiedName(xmlReader.GetXmiAttribute("type"), this.NameSpaceResolver);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:InformationItem")
                {
                    throw new XmlException($"The XmiType should be 'uml:InformationItem' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:InformationItem";
                }
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: InformationItem [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                ...
                var representedXmlAttribute = xmlReader.GetAttribute("represented") ?? xmlReader.GetAttribute("represented", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(representedXmlAttribute))
                {
                    var representedXmlAttributeValues = representedXmlAttribute.Split(SplitMultiReference, StringSplitOptions.RemoveEmptyEntries).ToList();
                    poco.MultiValueReferencePropertyIdentifiers.Add("represented", representedXmlAttributeValues);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "representation"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "represented"):
                                TryCollectMultiValueReferencePropertyIdentifiers(xmlReader, poco, "represented");
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "redefinitionContext"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of InformationItem at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"InformationItemReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: InformationItemReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
