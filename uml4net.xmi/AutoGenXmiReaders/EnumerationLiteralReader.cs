...
namespace uml4net.xmi.Readers
{
    ...
    public class EnumerationLiteralReader : XmiElementReader<IEnumerationLiteral>, IXmiElementReader<IEnumerationLiteral>
    {
        ...
        /// </param>
        public EnumerationLiteralReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<EnumerationLiteralReader>.Instance : loggerFactory.CreateLogger<EnumerationLiteralReader>();
        }
        ...
        public override IEnumerationLiteral Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: EnumerationLiteral [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }

                var enumerationXmlAttribute = xmlReader.GetAttribute("enumeration") ?? xmlReader.GetAttribute("enumeration", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(enumerationXmlAttribute))
                {
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            case (KnowNamespacePrefixes.Uml, "deployment"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "enumeration"):
                                CollectSingleValueReferencePropertyIdentifier(xmlReader, poco, "enumeration");
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "qualifiedName"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of EnumerationLiteral at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    this.logger.LogWarning("Not Supported: EnumerationLiteralReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
