// -------------------------------------------------------------------------------------------------
// <copyright file="StartClassifierBehaviorActionReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="StartClassifierBehaviorActionReader"/> is to read an instance of <see cref="IStartClassifierBehaviorAction"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class StartClassifierBehaviorActionReader : XmiElementReader<IStartClassifierBehaviorAction>, IXmiElementReader<IStartClassifierBehaviorAction>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<StartClassifierBehaviorActionReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="StartClassifierBehaviorActionReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public StartClassifierBehaviorActionReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<StartClassifierBehaviorActionReader>.Instance : loggerFactory.CreateLogger<StartClassifierBehaviorActionReader>();
        }

        /// <summary>
        /// Reads the <see cref="IStartClassifierBehaviorAction"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IStartClassifierBehaviorAction"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IStartClassifierBehaviorAction"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IStartClassifierBehaviorAction"/>
        /// </returns>
        public override IStartClassifierBehaviorAction Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IStartClassifierBehaviorAction poco = new StartClassifierBehaviorAction();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading StartClassifierBehaviorAction at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.GetAttribute("type", this.NameSpaceResolver.XmiNameSpace);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:StartClassifierBehaviorAction")
                {
                    throw new XmlException($"The XmiType should be 'uml:StartClassifierBehaviorAction' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:StartClassifierBehaviorAction";
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "StartClassifierBehaviorAction", poco.XmiId);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"StartClassifierBehaviorActionReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: StartClassifierBehaviorActionReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                                }
