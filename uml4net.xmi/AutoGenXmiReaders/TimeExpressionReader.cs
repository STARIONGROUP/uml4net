// -------------------------------------------------------------------------------------------------
// <copyright file="TimeExpressionReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="TimeExpressionReader"/> is to read an instance of <see cref="ITimeExpression"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class TimeExpressionReader : XmiElementReader<ITimeExpression>, IXmiElementReader<ITimeExpression>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<TimeExpressionReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="TimeExpressionReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public TimeExpressionReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<TimeExpressionReader>.Instance : loggerFactory.CreateLogger<TimeExpressionReader>();
        }

        /// <summary>
        /// Reads the <see cref="ITimeExpression"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="ITimeExpression"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="ITimeExpression"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="ITimeExpression"/>
        /// </returns>
        public override ITimeExpression Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            ITimeExpression poco = new TimeExpression();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading TimeExpression at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.ResolveQualifiedName(xmlReader.GetXmiAttribute("type"), this.NameSpaceResolver);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:TimeExpression")
                {
                    throw new XmlException($"The XmiType should be 'uml:TimeExpression' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:TimeExpression";
                }
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: TimeExpression [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }

                poco.Name = xmlReader.GetAttribute("name") ?? xmlReader.GetAttribute("name", this.NameSpaceResolver.UmlNameSpace);

                var observationXmlAttribute = xmlReader.GetAttribute("observation") ?? xmlReader.GetAttribute("observation", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(observationXmlAttribute))
                {
                ...
                while (xmlReader.Read())
                {
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        {
                            case (KnowNamespacePrefixes.Uml, "expr"):
                            ...
                            case (KnowNamespacePrefixes.Uml, "observation"):
                                TryCollectMultiValueReferencePropertyIdentifiers(xmlReader, poco, "observation");
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "qualifiedName"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of TimeExpression at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"TimeExpressionReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: TimeExpressionReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
