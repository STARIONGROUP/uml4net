...
namespace uml4net.xmi.Readers
{
    ...
    public class ExtensionReader : XmiElementReader<IExtension>, IXmiElementReader<IExtension>
    {
        ...
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<ExtensionReader> logger;
        ...
        /// </param>
        public ExtensionReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<ExtensionReader>.Instance : loggerFactory.CreateLogger<ExtensionReader>();
        }
        ...
        public override IExtension Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            ...
            {
                this.logger.LogTrace("reading Extension at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: Extension [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "ownedEnd"):
                                ...
                                {
                                    var ownedEndValue = (IExtensionEnd)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.ExtenderReaderRegistry, this.LoggerFactory, "uml:ExtensionEnd");
                            ...
                            case (KnowNamespacePrefixes.Uml, "isRequired"):
                            case (KnowNamespacePrefixes.Uml, "member"):
                            case (KnowNamespacePrefixes.Uml, "metaclass"):
                            ...
                            case (KnowNamespacePrefixes.Uml, "relatedElement"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of Extension at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"ExtensionReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: ExtensionReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
