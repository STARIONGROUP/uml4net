// -------------------------------------------------------------------------------------------------
// <copyright file="RemoveVariableValueActionReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="RemoveVariableValueActionReader"/> is to read an instance of <see cref="IRemoveVariableValueAction"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class RemoveVariableValueActionReader : XmiElementReader<IRemoveVariableValueAction>, IXmiElementReader<IRemoveVariableValueAction>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<RemoveVariableValueActionReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="RemoveVariableValueActionReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public RemoveVariableValueActionReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<RemoveVariableValueActionReader>.Instance : loggerFactory.CreateLogger<RemoveVariableValueActionReader>();
        }

        /// <summary>
        /// Reads the <see cref="IRemoveVariableValueAction"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IRemoveVariableValueAction"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IRemoveVariableValueAction"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IRemoveVariableValueAction"/>
        /// </returns>
        public override IRemoveVariableValueAction Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IRemoveVariableValueAction poco = new RemoveVariableValueAction();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading RemoveVariableValueAction at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.GetAttribute("type", this.NameSpaceResolver.XmiNameSpace);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:RemoveVariableValueAction")
                {
                    throw new XmlException($"The XmiType should be 'uml:RemoveVariableValueAction' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:RemoveVariableValueAction";
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "RemoveVariableValueAction", poco.XmiId);
                }
                ...
                var variableXmlAttribute = xmlReader.GetAttribute("variable") ?? xmlReader.GetAttribute("variable", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(variableXmlAttribute))
                {
                    poco.SingleValueReferencePropertyIdentifiers.Add("variable", variableXmlAttribute);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "variable"):
                                this.CollectSingleValueReferencePropertyIdentifier(xmlReader, poco, "variable");
                                break;
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"RemoveVariableValueActionReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: RemoveVariableValueActionReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                                }
