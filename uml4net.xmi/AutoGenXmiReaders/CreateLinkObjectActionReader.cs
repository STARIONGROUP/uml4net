// -------------------------------------------------------------------------------------------------
// <copyright file="CreateLinkObjectActionReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="CreateLinkObjectActionReader"/> is to read an instance of <see cref="ICreateLinkObjectAction"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class CreateLinkObjectActionReader : XmiElementReader<ICreateLinkObjectAction>, IXmiElementReader<ICreateLinkObjectAction>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<CreateLinkObjectActionReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="CreateLinkObjectActionReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public CreateLinkObjectActionReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<CreateLinkObjectActionReader>.Instance : loggerFactory.CreateLogger<CreateLinkObjectActionReader>();
        }

        /// <summary>
        /// Reads the <see cref="ICreateLinkObjectAction"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="ICreateLinkObjectAction"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="ICreateLinkObjectAction"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="ICreateLinkObjectAction"/>
        /// </returns>
        public override ICreateLinkObjectAction Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            ICreateLinkObjectAction poco = new CreateLinkObjectAction();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading CreateLinkObjectAction at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.GetAttribute("type", this.NameSpaceResolver.XmiNameSpace);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:CreateLinkObjectAction")
                {
                    throw new XmlException($"The XmiType should be 'uml:CreateLinkObjectAction' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:CreateLinkObjectAction";
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "CreateLinkObjectAction", poco.XmiId);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            case (KnowNamespacePrefixes.Uml, "activity"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "endData"):
                                var endDataValue = (ILinkEndCreationData)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.LoggerFactory, "uml:LinkEndCreationData");
                                poco.EndData.Add(endDataValue);
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "inputValue"):
                                var inputValueValue = (IInputPin)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.LoggerFactory, "uml:InputPin");
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"CreateLinkObjectActionReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: CreateLinkObjectActionReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                                }
