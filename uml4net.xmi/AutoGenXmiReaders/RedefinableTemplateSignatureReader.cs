...
namespace uml4net.xmi.Readers
{
    ...
    public class RedefinableTemplateSignatureReader : XmiElementReader<IRedefinableTemplateSignature>, IXmiElementReader<IRedefinableTemplateSignature>
    {
        ...
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<RedefinableTemplateSignatureReader> logger;
        ...
        /// </param>
        public RedefinableTemplateSignatureReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<RedefinableTemplateSignatureReader>.Instance : loggerFactory.CreateLogger<RedefinableTemplateSignatureReader>();
        }
        ...
        public override IRedefinableTemplateSignature Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            ...
            {
                this.logger.LogTrace("reading RedefinableTemplateSignature at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: RedefinableTemplateSignature [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                ...
                var extendedSignatureXmlAttribute = xmlReader.GetAttribute("extendedSignature") ?? xmlReader.GetAttribute("extendedSignature", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(extendedSignatureXmlAttribute))
                {
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "ownedParameter"):
                                ...
                                {
                                    var ownedParameterValue = (ITemplateParameter)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.ExtenderReaderRegistry, this.LoggerFactory, "uml:TemplateParameter");
                            ...
                            case (KnowNamespacePrefixes.Uml, "inheritedParameter"):
                            ...
                            case (KnowNamespacePrefixes.Uml, "redefinitionContext"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of RedefinableTemplateSignature at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"RedefinableTemplateSignatureReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: RedefinableTemplateSignatureReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
