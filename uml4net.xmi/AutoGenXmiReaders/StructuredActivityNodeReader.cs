// -------------------------------------------------------------------------------------------------
// <copyright file="StructuredActivityNodeReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="StructuredActivityNodeReader"/> is to read an instance of <see cref="IStructuredActivityNode"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class StructuredActivityNodeReader : XmiElementReader<IStructuredActivityNode>, IXmiElementReader<IStructuredActivityNode>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<StructuredActivityNodeReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="StructuredActivityNodeReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public StructuredActivityNodeReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<StructuredActivityNodeReader>.Instance : loggerFactory.CreateLogger<StructuredActivityNodeReader>();
        }

        /// <summary>
        /// Reads the <see cref="IStructuredActivityNode"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IStructuredActivityNode"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IStructuredActivityNode"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IStructuredActivityNode"/>
        /// </returns>
        public override IStructuredActivityNode Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IStructuredActivityNode poco = new StructuredActivityNode();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading StructuredActivityNode at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.GetAttribute("type", this.NameSpaceResolver.XmiNameSpace);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:StructuredActivityNode")
                {
                    throw new XmlException($"The XmiType should be 'uml:StructuredActivityNode' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:StructuredActivityNode";
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "StructuredActivityNode", poco.XmiId);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "edge"):
                                var edgeValue = (IActivityEdge)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.LoggerFactory);
                            ...
                            case (KnowNamespacePrefixes.Uml, "node"):
                                var nodeValue = (IActivityNode)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.LoggerFactory);
                            ...
                            case (KnowNamespacePrefixes.Uml, "structuredNodeInput"):
                                var structuredNodeInputValue = (IInputPin)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.LoggerFactory, "uml:InputPin");
                                poco.StructuredNodeInput.Add(structuredNodeInputValue);
                                break;
                            case (KnowNamespacePrefixes.Uml, "structuredNodeOutput"):
                                var structuredNodeOutputValue = (IOutputPin)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.LoggerFactory, "uml:OutputPin");
                                poco.StructuredNodeOutput.Add(structuredNodeOutputValue);
                                break;
                            case (KnowNamespacePrefixes.Uml, "variable"):
                                var variableValue = (IVariable)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.LoggerFactory, "uml:Variable");
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"StructuredActivityNodeReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: StructuredActivityNodeReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                                }
