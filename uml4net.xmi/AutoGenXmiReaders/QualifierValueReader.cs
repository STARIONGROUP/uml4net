// -------------------------------------------------------------------------------------------------
// <copyright file="QualifierValueReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    public class QualifierValueReader : XmiElementReader<IQualifierValue>, IXmiElementReader<IQualifierValue>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<QualifierValueReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="QualifierValueReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public QualifierValueReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<QualifierValueReader>.Instance : loggerFactory.CreateLogger<QualifierValueReader>();
        }

        /// <summary>
        /// Reads the <see cref="IQualifierValue"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IQualifierValue"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IQualifierValue"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IQualifierValue"/>
        /// </returns>
        public override IQualifierValue Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            {
                this.logger.LogTrace("reading QualifierValue at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.ResolveQualifiedName(xmlReader.GetXmiAttribute("type"), this.NameSpaceResolver);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:QualifierValue")
                {
                    throw new XmlException($"The XmiType should be 'uml:QualifierValue' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:QualifierValue";
                }
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: QualifierValue [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "owner"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of QualifierValue at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"QualifierValueReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: QualifierValueReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
