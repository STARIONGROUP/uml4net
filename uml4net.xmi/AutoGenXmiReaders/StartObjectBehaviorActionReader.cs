...
namespace uml4net.xmi.Readers
{
    ...
    public class StartObjectBehaviorActionReader : XmiElementReader<IStartObjectBehaviorAction>, IXmiElementReader<IStartObjectBehaviorAction>
    {
        ...
        /// </param>
        public StartObjectBehaviorActionReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
        ...
        public override IStartObjectBehaviorAction Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: StartObjectBehaviorAction [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                ...
                {
                    poco.IsSynchronous = XmlConvert.ToBoolean(isSynchronousXmlAttribute);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "argument"):
                                ...
                                {
                                    var argumentValue = (IInputPin)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.ExtenderReaderRegistry, this.LoggerFactory, "uml:InputPin");
                            ...
                            case (KnowNamespacePrefixes.Uml, "isSynchronous"):
                                var isSynchronousValue = xmlReader.ReadElementContentAsStringInPlace();

                                if (!string.IsNullOrWhiteSpace(isSynchronousValue))
                                {
                                    poco.IsSynchronous = XmlConvert.ToBoolean(isSynchronousValue);
                                }
                            ...
                            case (KnowNamespacePrefixes.Uml, "onPort"):
                                CollectSingleValueReferencePropertyIdentifier(xmlReader, poco, "onPort");
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "redefinitionContext"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of StartObjectBehaviorAction at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
