// -------------------------------------------------------------------------------------------------
// <copyright file="StartObjectBehaviorActionReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="StartObjectBehaviorActionReader"/> is to read an instance of <see cref="IStartObjectBehaviorAction"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class StartObjectBehaviorActionReader : XmiElementReader<IStartObjectBehaviorAction>, IXmiElementReader<IStartObjectBehaviorAction>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<StartObjectBehaviorActionReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="StartObjectBehaviorActionReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public StartObjectBehaviorActionReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<StartObjectBehaviorActionReader>.Instance : loggerFactory.CreateLogger<StartObjectBehaviorActionReader>();
        }

        /// <summary>
        /// Reads the <see cref="IStartObjectBehaviorAction"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IStartObjectBehaviorAction"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IStartObjectBehaviorAction"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IStartObjectBehaviorAction"/>
        /// </returns>
        public override IStartObjectBehaviorAction Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IStartObjectBehaviorAction poco = new StartObjectBehaviorAction();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading StartObjectBehaviorAction at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.GetAttribute("type", this.NameSpaceResolver.XmiNameSpace);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:StartObjectBehaviorAction")
                {
                    throw new XmlException($"The XmiType should be 'uml:StartObjectBehaviorAction' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:StartObjectBehaviorAction";
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "StartObjectBehaviorAction", poco.XmiId);
                }
                ...
                var isSynchronousXmlAttribute = xmlReader.GetAttribute("isSynchronous") ?? xmlReader.GetAttribute("isSynchronous", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(isSynchronousXmlAttribute))
                {
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "argument"):
                                var argumentValue = (IInputPin)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.LoggerFactory, "uml:InputPin");
                            ...
                            case (KnowNamespacePrefixes.Uml, "isLocallyReentrant"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "isSynchronous"):
                                var isSynchronousValue = xmlReader.ReadElementContentAsString();

                                if (!string.IsNullOrWhiteSpace(isSynchronousValue))
                                {
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"StartObjectBehaviorActionReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: StartObjectBehaviorActionReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                                }
