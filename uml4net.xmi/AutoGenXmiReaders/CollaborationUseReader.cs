// -------------------------------------------------------------------------------------------------
// <copyright file="CollaborationUseReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    public class CollaborationUseReader : XmiElementReader<ICollaborationUse>, IXmiElementReader<ICollaborationUse>
    {
        ...
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<CollaborationUseReader> logger;
        ...
        /// </param>
        public CollaborationUseReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<CollaborationUseReader>.Instance : loggerFactory.CreateLogger<CollaborationUseReader>();
        }
        ...
        public override ICollaborationUse Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            ...
            ICollaborationUse poco = new CollaborationUse();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading CollaborationUse at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.ResolveQualifiedName(xmlReader.GetXmiAttribute("type"), this.NameSpaceResolver);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:CollaborationUse")
                {
                    throw new XmlException($"The XmiType should be 'uml:CollaborationUse' while it is {xmiType}");
                }
                else
                {
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: CollaborationUse [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "ownedComment"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "roleBinding"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "roleBinding", poco.RoleBinding.Count))
                                {
                                    var roleBindingValue = (IDependency)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.ExtenderReaderRegistry, this.LoggerFactory, "uml:Dependency");
                            ...
                            case (KnowNamespacePrefixes.Uml, "qualifiedName"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of CollaborationUse at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"CollaborationUseReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: CollaborationUseReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
