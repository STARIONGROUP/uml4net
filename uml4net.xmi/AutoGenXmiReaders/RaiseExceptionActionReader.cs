// -------------------------------------------------------------------------------------------------
// <copyright file="RaiseExceptionActionReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="RaiseExceptionActionReader"/> is to read an instance of <see cref="IRaiseExceptionAction"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class RaiseExceptionActionReader : XmiElementReader<IRaiseExceptionAction>, IXmiElementReader<IRaiseExceptionAction>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<RaiseExceptionActionReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="RaiseExceptionActionReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public RaiseExceptionActionReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<RaiseExceptionActionReader>.Instance : loggerFactory.CreateLogger<RaiseExceptionActionReader>();
        }

        /// <summary>
        /// Reads the <see cref="IRaiseExceptionAction"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IRaiseExceptionAction"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IRaiseExceptionAction"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IRaiseExceptionAction"/>
        /// </returns>
        public override IRaiseExceptionAction Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IRaiseExceptionAction poco = new RaiseExceptionAction();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading RaiseExceptionAction at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.GetAttribute("type", this.NameSpaceResolver.XmiNameSpace);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:RaiseExceptionAction")
                {
                    throw new XmlException($"The XmiType should be 'uml:RaiseExceptionAction' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:RaiseExceptionAction";
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "RaiseExceptionAction", poco.XmiId);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            case (KnowNamespacePrefixes.Uml, "activity"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "exception"):
                                var exceptionValue = (IInputPin)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.LoggerFactory, "uml:InputPin");
                                poco.Exception.Add(exceptionValue);
                                break;
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"RaiseExceptionActionReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: RaiseExceptionActionReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                                }
