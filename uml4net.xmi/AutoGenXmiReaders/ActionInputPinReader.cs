...
namespace uml4net.xmi.Readers
{
    ...
    public class ActionInputPinReader : XmiElementReader<IActionInputPin>, IXmiElementReader<IActionInputPin>
    {
        ...
        /// </param>
        public ActionInputPinReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, ILoggerFactory loggerFactory)
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmlReader.LocalName)
                        {
                            case "activity":
                                ...
                                break;
                            case "fromAction":
                                var fromActionValue = (IAction)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.LoggerFactory);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"ActionInputPinReader: {xmlReader.LocalName} at line:position {defaultLineInfo.LineNumber}:{defaultLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.Logger.LogWarning("Not Supported: ActionInputPinReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, defaultLineInfo.LineNumber, defaultLineInfo.LinePosition);
                                }
                                break;
                        }
                    }
