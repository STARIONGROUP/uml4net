...
namespace uml4net.xmi.Readers
{
    ...
    public class FinalStateReader : XmiElementReader<IFinalState>, IXmiElementReader<IFinalState>
    {
        ...
        /// </param>
        public FinalStateReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
        ...
        public override IFinalState Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: FinalState [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            case (KnowNamespacePrefixes.Uml, "connection"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "connection", poco.Connection.Count))
                                {
                            ...
                            case (KnowNamespacePrefixes.Uml, "deferrableTrigger"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "deferrableTrigger", poco.DeferrableTrigger.Count))
                                {
                            ...
                            case (KnowNamespacePrefixes.Uml, "isOrthogonal"):
                            case (KnowNamespacePrefixes.Uml, "isSimple"):
                            case (KnowNamespacePrefixes.Uml, "isSubmachineState"):
                            ...
                            case (KnowNamespacePrefixes.Uml, "redefinitionContext"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of FinalState at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
