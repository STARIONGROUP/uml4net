...
namespace uml4net.xmi.Readers
{
    ...
    public class SlotReader : XmiElementReader<ISlot>, IXmiElementReader<ISlot>
    {
        ...
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<SlotReader> logger;
        ...
        /// </param>
        public SlotReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<SlotReader>.Instance : loggerFactory.CreateLogger<SlotReader>();
        }
        ...
        /// The name of the document that contains the <see cref="ISlot"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="ISlot"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="ISlot"/>
        /// </returns>
        public override ISlot Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            {
                this.logger.LogTrace("reading Slot at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: Slot [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }

                var definingFeatureXmlAttribute = xmlReader.GetAttribute("definingFeature") ?? xmlReader.GetAttribute("definingFeature", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(definingFeatureXmlAttribute))
                {
                    poco.SingleValueReferencePropertyIdentifiers.Add("definingFeature", definingFeatureXmlAttribute);
                }

                var owningInstanceXmlAttribute = xmlReader.GetAttribute("owningInstance") ?? xmlReader.GetAttribute("owningInstance", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(owningInstanceXmlAttribute))
                {
                ...
                while (xmlReader.Read())
                {
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        {
                            case (KnowNamespacePrefixes.Uml, "definingFeature"):
                                CollectSingleValueReferencePropertyIdentifier(xmlReader, poco, "definingFeature");
                                break;
                            case (KnowNamespacePrefixes.Uml, "ownedComment"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "owningInstance"):
                                CollectSingleValueReferencePropertyIdentifier(xmlReader, poco, "owningInstance");
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "owner"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of Slot at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"SlotReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: SlotReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
