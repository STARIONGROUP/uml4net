...
namespace uml4net.xmi.Readers
{
    ...
    public class ComponentRealizationReader : XmiElementReader<IComponentRealization>, IXmiElementReader<IComponentRealization>
    {
        ...
        /// </param>
        public ComponentRealizationReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
        ...
        public override IComponentRealization Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: ComponentRealization [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                ...
                if (!string.IsNullOrWhiteSpace(abstractionXmlAttribute))
                {
                ...
                if (!string.IsNullOrWhiteSpace(realizingClassifierXmlAttribute))
                {
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            case (KnowNamespacePrefixes.Uml, "abstraction"):
                                CollectSingleValueReferencePropertyIdentifier(xmlReader, poco, "abstraction");
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "realizingClassifier"):
                                TryCollectMultiValueReferencePropertyIdentifiers(xmlReader, poco, "realizingClassifier");
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "target"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of ComponentRealization at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
