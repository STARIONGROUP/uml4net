// -------------------------------------------------------------------------------------------------
// <copyright file="DeviceReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="DeviceReader"/> is to read an instance of <see cref="IDevice"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class DeviceReader : XmiElementReader<IDevice>, IXmiElementReader<IDevice>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<DeviceReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="DeviceReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public DeviceReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<DeviceReader>.Instance : loggerFactory.CreateLogger<DeviceReader>();
        }

        /// <summary>
        /// Reads the <see cref="IDevice"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IDevice"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IDevice"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IDevice"/>
        /// </returns>
        public override IDevice Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IDevice poco = new Device();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading Device at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.ResolveQualifiedName(xmlReader.GetXmiAttribute("type"), this.NameSpaceResolver);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:Device")
                {
                    throw new XmlException($"The XmiType should be 'uml:Device' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:Device";
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: Device [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "deployment"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "deployment", poco.Deployment.Count))
                                {
                            ...
                            case (KnowNamespacePrefixes.Uml, "nestedClassifier"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "nestedNode"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "nestedNode", poco.NestedNode.Count))
                                {
                                    var nestedNodeValue = (INode)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.ExtenderReaderRegistry, this.LoggerFactory, "uml:Node");
                            ...
                            case (KnowNamespacePrefixes.Uml, "deployedElement"):
                            ...
                            case (KnowNamespacePrefixes.Uml, "superClass"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of Device at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"DeviceReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: DeviceReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
