...
namespace uml4net.xmi.Readers
{
    ...
    public class SendSignalActionReader : XmiElementReader<ISendSignalAction>, IXmiElementReader<ISendSignalAction>
    {
        ...
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<SendSignalActionReader> logger;
        ...
        /// </param>
        public SendSignalActionReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<SendSignalActionReader>.Instance : loggerFactory.CreateLogger<SendSignalActionReader>();
        }
        ...
        /// The name of the document that contains the <see cref="ISendSignalAction"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="ISendSignalAction"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="ISendSignalAction"/>
        /// </returns>
        public override ISendSignalAction Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            {
                this.logger.LogTrace("reading SendSignalAction at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "SendSignalAction", poco.XmiId);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"SendSignalActionReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: SendSignalActionReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                                }
