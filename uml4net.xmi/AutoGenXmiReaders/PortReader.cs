...
namespace uml4net.xmi.Readers.StructuredClassifiers
{
    ...
    public class PortReader : XmiElementReader<IPort>, IXmiElementReader<IPort>
    {
        ...
        /// </param>
        public PortReader(IXmiReaderCache cache, ILoggerFactory loggerFactory)
        ...
        /// </returns>
        public override IPort Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                var isBehaviorXmlAttribute = xmlReader.GetAttribute("isBehavior");
                if (!string.IsNullOrEmpty(isBehaviorXmlAttribute))
                {
                    poco.IsBehavior = bool.Parse(isBehaviorXmlAttribute);
                }

                var isConjugatedXmlAttribute = xmlReader.GetAttribute("isConjugated");
                if (!string.IsNullOrEmpty(isConjugatedXmlAttribute))
                {
                ...
                var isServiceXmlAttribute = xmlReader.GetAttribute("isService");
                if (!string.IsNullOrEmpty(isServiceXmlAttribute))
                {
                ...
                var protocolXmlAttribute = xmlReader.GetAttribute("protocol");
                if (!string.IsNullOrEmpty(protocolXmlAttribute))
                {
                    poco.SingleValueReferencePropertyIdentifiers.Add("protocol", protocolXmlAttribute);
                }

                var redefinedPortXmlAttribute = xmlReader.GetAttribute("redefinedPort");
                if (!string.IsNullOrEmpty(redefinedPortXmlAttribute))
                {
                ...
                var redefinedPort = new List<string>();
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "interface":
                                ...
                                break;
                            case "isBehavior":
                                var isBehaviorXmlElement = xmlReader.ReadElementContentAsString();
                                if (!string.IsNullOrEmpty(isBehaviorXmlElement))
                                {
                                    poco.IsBehavior = bool.Parse(isBehaviorXmlElement);
                                }
                                break;
                            case "isConjugated":
                                var isConjugatedXmlElement = xmlReader.ReadElementContentAsString();
                                if (!string.IsNullOrEmpty(isConjugatedXmlElement))
                                {
                                    poco.IsConjugated = bool.Parse(isConjugatedXmlElement);
                                }
                                break;
                            case "isDerived":
                            ...
                            case "isReadOnly":
                                ...
                                break;
                            case "isService":
                                var isServiceXmlElement = xmlReader.ReadElementContentAsString();
                                if (!string.IsNullOrEmpty(isServiceXmlElement))
                                {
                                    poco.IsService = bool.Parse(isServiceXmlElement);
                                }
                                break;
                            case "isStatic":
                            ...
                            case "qualifier":
                                ...
                                break;
                            case "redefinedPort":
                                this.CollectMultiValueReferencePropertyIdentifiers(xmlReader, redefinedPort, "redefinedPort");
                                break;
                            ...
                            default:
                                ...
                                throw new NotSupportedException($"PortReader: {xmlReader.LocalName} at line:position {defaultLineInfo.LineNumber}:{defaultLineInfo.LinePosition}");
                        }
                    }
                }

                if (redefinedPort.Count > 0)
                {
                    poco.MultiValueReferencePropertyIdentifiers.Add("redefinedPort", redefinedPort);
                }
