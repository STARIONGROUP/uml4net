        ...
        public override IPort Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: Port [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                ...
                {
                    poco.IsBehavior = XmlConvert.ToBoolean(isBehaviorXmlAttribute);
                }
                ...
                {
                    poco.IsConjugated = XmlConvert.ToBoolean(isConjugatedXmlAttribute);
                }
                ...
                {
                    poco.IsService = XmlConvert.ToBoolean(isServiceXmlAttribute);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "isBehavior"):
                                var isBehaviorValue = xmlReader.ReadElementContentAsStringInPlace();

                                if (!string.IsNullOrWhiteSpace(isBehaviorValue))
                                {
                                    poco.IsBehavior = XmlConvert.ToBoolean(isBehaviorValue);
                                }

                                break;
                            case (KnowNamespacePrefixes.Uml, "isConjugated"):
                                var isConjugatedValue = xmlReader.ReadElementContentAsStringInPlace();

                                if (!string.IsNullOrWhiteSpace(isConjugatedValue))
                                {
                                    poco.IsConjugated = XmlConvert.ToBoolean(isConjugatedValue);
                                }
                            ...
                            case (KnowNamespacePrefixes.Uml, "isService"):
                                var isServiceValue = xmlReader.ReadElementContentAsStringInPlace();

                                if (!string.IsNullOrWhiteSpace(isServiceValue))
                                {
                                    poco.IsService = XmlConvert.ToBoolean(isServiceValue);
                                }
                            ...
                            case (KnowNamespacePrefixes.Uml, "protocol"):
                                CollectSingleValueReferencePropertyIdentifier(xmlReader, poco, "protocol");
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "redefinedPort"):
                                TryCollectMultiValueReferencePropertyIdentifiers(xmlReader, poco, "redefinedPort");
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "upper"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of Port at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
