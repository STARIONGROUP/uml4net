...
namespace uml4net.xmi.Readers
{
    ...
    public class ParameterReader : XmiElementReader<IParameter>, IXmiElementReader<IParameter>
    {
        ...
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<ParameterReader> logger;
        ...
        /// </param>
        public ParameterReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<ParameterReader>.Instance : loggerFactory.CreateLogger<ParameterReader>();
        }
        ...
        public override IParameter Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                {
                    poco.IsException = XmlConvert.ToBoolean(isExceptionXmlAttribute);
                }
                ...
                {
                    poco.IsStream = XmlConvert.ToBoolean(isStreamXmlAttribute);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "direction"):
                                var directionValue = xmlReader.ReadElementContentAsStringInPlace();
                            ...
                            case (KnowNamespacePrefixes.Uml, "effect"):
                                var effectValue = xmlReader.ReadElementContentAsStringInPlace();
                            ...
                            case (KnowNamespacePrefixes.Uml, "isException"):
                                var isExceptionValue = xmlReader.ReadElementContentAsStringInPlace();

                                if (!string.IsNullOrWhiteSpace(isExceptionValue))
                                {
                                    poco.IsException = XmlConvert.ToBoolean(isExceptionValue);
                                }
                            ...
                            case (KnowNamespacePrefixes.Uml, "isStream"):
                                var isStreamValue = xmlReader.ReadElementContentAsStringInPlace();

                                if (!string.IsNullOrWhiteSpace(isStreamValue))
                                {
                                    poco.IsStream = XmlConvert.ToBoolean(isStreamValue);
                                }
                            ...
                            case (KnowNamespacePrefixes.Uml, "owningTemplateParameter"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "parameterSet"):
                                TryCollectMultiValueReferencePropertyIdentifiers(xmlReader, poco, "parameterSet");
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "upper"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of Parameter at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    this.logger.LogWarning("Not Supported: ParameterReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
