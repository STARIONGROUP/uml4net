// -------------------------------------------------------------------------------------------------
// <copyright file="ManifestationReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="ManifestationReader"/> is to read an instance of <see cref="IManifestation"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class ManifestationReader : XmiElementReader<IManifestation>, IXmiElementReader<IManifestation>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<ManifestationReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="ManifestationReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public ManifestationReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<ManifestationReader>.Instance : loggerFactory.CreateLogger<ManifestationReader>();
        }

        /// <summary>
        /// Reads the <see cref="IManifestation"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IManifestation"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IManifestation"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IManifestation"/>
        /// </returns>
        public override IManifestation Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IManifestation poco = new Manifestation();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading Manifestation at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.ResolveQualifiedName(xmlReader.GetXmiAttribute("type"), this.NameSpaceResolver);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:Manifestation")
                {
                    throw new XmlException($"The XmiType should be 'uml:Manifestation' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:Manifestation";
                }
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: Manifestation [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }

                var clientXmlAttribute = xmlReader.GetAttribute("client") ?? xmlReader.GetAttribute("client", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(clientXmlAttribute))
                {
                ...
                if (!string.IsNullOrWhiteSpace(supplierXmlAttribute))
                {
                ...
                var utilizedElementXmlAttribute = xmlReader.GetAttribute("utilizedElement") ?? xmlReader.GetAttribute("utilizedElement", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(utilizedElementXmlAttribute))
                {
                    poco.SingleValueReferencePropertyIdentifiers.Add("utilizedElement", utilizedElementXmlAttribute);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "target"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of Manifestation at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"ManifestationReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: ManifestationReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
