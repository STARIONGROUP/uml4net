// -------------------------------------------------------------------------------------------------
// <copyright file="IncludeReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="IncludeReader"/> is to read an instance of <see cref="IInclude"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class IncludeReader : XmiElementReader<IInclude>, IXmiElementReader<IInclude>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<IncludeReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="IncludeReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public IncludeReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<IncludeReader>.Instance : loggerFactory.CreateLogger<IncludeReader>();
        }

        /// <summary>
        /// Reads the <see cref="IInclude"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IInclude"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IInclude"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IInclude"/>
        /// </returns>
        public override IInclude Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IInclude poco = new Include();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading Include at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.ResolveQualifiedName(xmlReader.GetXmiAttribute("type"), this.NameSpaceResolver);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:Include")
                {
                    throw new XmlException($"The XmiType should be 'uml:Include' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:Include";
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: Include [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }

                var additionXmlAttribute = xmlReader.GetAttribute("addition") ?? xmlReader.GetAttribute("addition", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(additionXmlAttribute))
                {
                    poco.SingleValueReferencePropertyIdentifiers.Add("addition", additionXmlAttribute);
                }

                var includingCaseXmlAttribute = xmlReader.GetAttribute("includingCase") ?? xmlReader.GetAttribute("includingCase", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(includingCaseXmlAttribute))
                {
                    poco.SingleValueReferencePropertyIdentifiers.Add("includingCase", includingCaseXmlAttribute);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "target"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of Include at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"IncludeReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: IncludeReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
