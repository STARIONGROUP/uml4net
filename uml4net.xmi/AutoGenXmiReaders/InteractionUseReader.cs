...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="InteractionUseReader"/> is to read an instance of <see cref="IInteractionUse"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class InteractionUseReader : XmiElementReader<IInteractionUse>, IXmiElementReader<IInteractionUse>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<InteractionUseReader> logger;
        ...
        /// </param>
        public InteractionUseReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<InteractionUseReader>.Instance : loggerFactory.CreateLogger<InteractionUseReader>();
        }

        /// <summary>
        /// Reads the <see cref="IInteractionUse"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IInteractionUse"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IInteractionUse"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IInteractionUse"/>
        /// </returns>
        public override IInteractionUse Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IInteractionUse poco = new InteractionUse();
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:InteractionUse")
                {
                    throw new XmlException($"The XmiType should be 'uml:InteractionUse' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:InteractionUse";
                }
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: InteractionUse [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "qualifiedName"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of InteractionUse at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    this.logger.LogWarning("Not Supported: InteractionUseReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
