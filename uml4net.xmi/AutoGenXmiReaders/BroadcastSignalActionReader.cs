// -------------------------------------------------------------------------------------------------
// <copyright file="BroadcastSignalActionReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="BroadcastSignalActionReader"/> is to read an instance of <see cref="IBroadcastSignalAction"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class BroadcastSignalActionReader : XmiElementReader<IBroadcastSignalAction>, IXmiElementReader<IBroadcastSignalAction>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<BroadcastSignalActionReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="BroadcastSignalActionReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public BroadcastSignalActionReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<BroadcastSignalActionReader>.Instance : loggerFactory.CreateLogger<BroadcastSignalActionReader>();
        }

        /// <summary>
        /// Reads the <see cref="IBroadcastSignalAction"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IBroadcastSignalAction"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IBroadcastSignalAction"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IBroadcastSignalAction"/>
        /// </returns>
        public override IBroadcastSignalAction Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IBroadcastSignalAction poco = new BroadcastSignalAction();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading BroadcastSignalAction at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.GetAttribute("type", this.NameSpaceResolver.XmiNameSpace);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:BroadcastSignalAction")
                {
                    throw new XmlException($"The XmiType should be 'uml:BroadcastSignalAction' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:BroadcastSignalAction";
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "BroadcastSignalAction", poco.XmiId);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"BroadcastSignalActionReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: BroadcastSignalActionReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                                }
