...
namespace uml4net.xmi.Readers
{
    ...
    public class TemplateParameterSubstitutionReader : XmiElementReader<ITemplateParameterSubstitution>, IXmiElementReader<ITemplateParameterSubstitution>
    {
        ...
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<TemplateParameterSubstitutionReader> logger;
        ...
        /// </param>
        public TemplateParameterSubstitutionReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
        ...
        /// The name of the document that contains the <see cref="ITemplateParameterSubstitution"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="ITemplateParameterSubstitution"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="ITemplateParameterSubstitution"/>
        /// </returns>
        public override ITemplateParameterSubstitution Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: TemplateParameterSubstitution [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "owner"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of TemplateParameterSubstitution at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"TemplateParameterSubstitutionReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: TemplateParameterSubstitutionReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
