// -------------------------------------------------------------------------------------------------
// <copyright file="ConnectableElementTemplateParameterReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="ConnectableElementTemplateParameterReader"/> is to read an instance of <see cref="IConnectableElementTemplateParameter"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class ConnectableElementTemplateParameterReader : XmiElementReader<IConnectableElementTemplateParameter>, IXmiElementReader<IConnectableElementTemplateParameter>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<ConnectableElementTemplateParameterReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="ConnectableElementTemplateParameterReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public ConnectableElementTemplateParameterReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<ConnectableElementTemplateParameterReader>.Instance : loggerFactory.CreateLogger<ConnectableElementTemplateParameterReader>();
        }

        /// <summary>
        /// Reads the <see cref="IConnectableElementTemplateParameter"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IConnectableElementTemplateParameter"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IConnectableElementTemplateParameter"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IConnectableElementTemplateParameter"/>
        /// </returns>
        public override IConnectableElementTemplateParameter Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IConnectableElementTemplateParameter poco = new ConnectableElementTemplateParameter();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading ConnectableElementTemplateParameter at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.ResolveQualifiedName(xmlReader.GetXmiAttribute("type"), this.NameSpaceResolver);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:ConnectableElementTemplateParameter")
                {
                    throw new XmlException($"The XmiType should be 'uml:ConnectableElementTemplateParameter' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:ConnectableElementTemplateParameter";
                }
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: ConnectableElementTemplateParameter [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "owner"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of ConnectableElementTemplateParameter at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"ConnectableElementTemplateParameterReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: ConnectableElementTemplateParameterReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
