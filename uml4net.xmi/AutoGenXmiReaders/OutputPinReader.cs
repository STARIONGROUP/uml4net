...
namespace uml4net.xmi.Readers
{
    ...
    public class OutputPinReader : XmiElementReader<IOutputPin>, IXmiElementReader<IOutputPin>
    {
        ...
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<OutputPinReader> logger;
        ...
        /// </param>
        public OutputPinReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<OutputPinReader>.Instance : loggerFactory.CreateLogger<OutputPinReader>();
        }
        ...
        /// The name of the document that contains the <see cref="IOutputPin"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IOutputPin"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IOutputPin"/>
        /// </returns>
        public override IOutputPin Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            {
                this.logger.LogTrace("reading OutputPin at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: OutputPin [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "upper"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of OutputPin at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"OutputPinReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: OutputPinReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
