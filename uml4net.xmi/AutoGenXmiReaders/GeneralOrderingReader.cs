// -------------------------------------------------------------------------------------------------
// <copyright file="GeneralOrderingReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="GeneralOrderingReader"/> is to read an instance of <see cref="IGeneralOrdering"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class GeneralOrderingReader : XmiElementReader<IGeneralOrdering>, IXmiElementReader<IGeneralOrdering>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<GeneralOrderingReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="GeneralOrderingReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public GeneralOrderingReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<GeneralOrderingReader>.Instance : loggerFactory.CreateLogger<GeneralOrderingReader>();
        }

        /// <summary>
        /// Reads the <see cref="IGeneralOrdering"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IGeneralOrdering"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IGeneralOrdering"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IGeneralOrdering"/>
        /// </returns>
        public override IGeneralOrdering Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IGeneralOrdering poco = new GeneralOrdering();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading GeneralOrdering at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.ResolveQualifiedName(xmlReader.GetXmiAttribute("type"), this.NameSpaceResolver);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:GeneralOrdering")
                {
                    throw new XmlException($"The XmiType should be 'uml:GeneralOrdering' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:GeneralOrdering";
                }
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: GeneralOrdering [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }

                var afterXmlAttribute = xmlReader.GetAttribute("after") ?? xmlReader.GetAttribute("after", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(afterXmlAttribute))
                {
                    poco.SingleValueReferencePropertyIdentifiers.Add("after", afterXmlAttribute);
                }

                var beforeXmlAttribute = xmlReader.GetAttribute("before") ?? xmlReader.GetAttribute("before", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(beforeXmlAttribute))
                {
                ...
                while (xmlReader.Read())
                {
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        {
                            case (KnowNamespacePrefixes.Uml, "after"):
                                CollectSingleValueReferencePropertyIdentifier(xmlReader, poco, "after");
                                break;
                            case (KnowNamespacePrefixes.Uml, "before"):
                                CollectSingleValueReferencePropertyIdentifier(xmlReader, poco, "before");
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "qualifiedName"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of GeneralOrdering at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"GeneralOrderingReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: GeneralOrderingReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
