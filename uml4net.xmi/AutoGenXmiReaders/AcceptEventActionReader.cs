// -------------------------------------------------------------------------------------------------
// <copyright file="AcceptEventActionReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="AcceptEventActionReader"/> is to read an instance of <see cref="IAcceptEventAction"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class AcceptEventActionReader : XmiElementReader<IAcceptEventAction>, IXmiElementReader<IAcceptEventAction>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<AcceptEventActionReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="AcceptEventActionReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public AcceptEventActionReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<AcceptEventActionReader>.Instance : loggerFactory.CreateLogger<AcceptEventActionReader>();
        }

        /// <summary>
        /// Reads the <see cref="IAcceptEventAction"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IAcceptEventAction"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IAcceptEventAction"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IAcceptEventAction"/>
        /// </returns>
        public override IAcceptEventAction Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IAcceptEventAction poco = new AcceptEventAction();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading AcceptEventAction at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.GetAttribute("type", this.NameSpaceResolver.XmiNameSpace);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:AcceptEventAction")
                {
                    throw new XmlException($"The XmiType should be 'uml:AcceptEventAction' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:AcceptEventAction";
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "AcceptEventAction", poco.XmiId);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"AcceptEventActionReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: AcceptEventActionReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                                }
