...
namespace uml4net.xmi.Readers
{
    ...
    public class InstanceValueReader : XmiElementReader<IInstanceValue>, IXmiElementReader<IInstanceValue>
    {
        ...
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<InstanceValueReader> logger;
        ...
        /// </param>
        public InstanceValueReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<InstanceValueReader>.Instance : loggerFactory.CreateLogger<InstanceValueReader>();
        }
        ...
        /// The name of the document that contains the <see cref="IInstanceValue"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IInstanceValue"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IInstanceValue"/>
        /// </returns>
        public override IInstanceValue Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            {
                this.logger.LogTrace("reading InstanceValue at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: InstanceValue [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }

                var instanceXmlAttribute = xmlReader.GetAttribute("instance") ?? xmlReader.GetAttribute("instance", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(instanceXmlAttribute))
                {
                    poco.SingleValueReferencePropertyIdentifiers.Add("instance", instanceXmlAttribute);
                }
                ...
                while (xmlReader.Read())
                {
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        {
                            case (KnowNamespacePrefixes.Uml, "instance"):
                                CollectSingleValueReferencePropertyIdentifier(xmlReader, poco, "instance");
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "qualifiedName"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of InstanceValue at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"InstanceValueReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: InstanceValueReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
