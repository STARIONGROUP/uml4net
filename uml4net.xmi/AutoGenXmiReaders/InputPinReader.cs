// -------------------------------------------------------------------------------------------------
// <copyright file="InputPinReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="InputPinReader"/> is to read an instance of <see cref="IInputPin"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class InputPinReader : XmiElementReader<IInputPin>, IXmiElementReader<IInputPin>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<InputPinReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="InputPinReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public InputPinReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<InputPinReader>.Instance : loggerFactory.CreateLogger<InputPinReader>();
        }

        /// <summary>
        /// Reads the <see cref="IInputPin"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IInputPin"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IInputPin"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IInputPin"/>
        /// </returns>
        public override IInputPin Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IInputPin poco = new InputPin();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading InputPin at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.ResolveQualifiedName(xmlReader.GetXmiAttribute("type"), this.NameSpaceResolver);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:InputPin")
                {
                    throw new XmlException($"The XmiType should be 'uml:InputPin' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:InputPin";
                }
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: InputPin [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "isControl"):
                                var isControlValue = xmlReader.ReadElementContentAsStringInPlace();
                            ...
                            case (KnowNamespacePrefixes.Uml, "lower"):
                            ...
                            case (KnowNamespacePrefixes.Uml, "upper"):
                                // serialized derived data (XMI 2.5.1 clause 7.8.10) is computed by uml4net, not read
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of InputPin at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"InputPinReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: InputPinReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
