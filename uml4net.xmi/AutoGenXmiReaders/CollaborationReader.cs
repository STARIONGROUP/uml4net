// -------------------------------------------------------------------------------------------------
// <copyright file="CollaborationReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="CollaborationReader"/> is to read an instance of <see cref="ICollaboration"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class CollaborationReader : XmiElementReader<ICollaboration>, IXmiElementReader<ICollaboration>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<CollaborationReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="CollaborationReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public CollaborationReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<CollaborationReader>.Instance : loggerFactory.CreateLogger<CollaborationReader>();
        }
        ...
        /// The name of the document that contains the <see cref="ICollaboration"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="ICollaboration"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="ICollaboration"/>
        /// </returns>
        public override ICollaboration Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            ICollaboration poco = new Collaboration();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading Collaboration at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.ResolveQualifiedName(xmlReader.GetXmiAttribute("type"), this.NameSpaceResolver);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:Collaboration")
                {
                    throw new XmlException($"The XmiType should be 'uml:Collaboration' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:Collaboration";
                }
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: Collaboration [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                ...
                var collaborationRoleXmlAttribute = xmlReader.GetAttribute("collaborationRole") ?? xmlReader.GetAttribute("collaborationRole", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(collaborationRoleXmlAttribute))
                {
                    var collaborationRoleXmlAttributeValues = collaborationRoleXmlAttribute.Split(SplitMultiReference, StringSplitOptions.RemoveEmptyEntries).ToList();
                    poco.MultiValueReferencePropertyIdentifiers.Add("collaborationRole", collaborationRoleXmlAttributeValues);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            case (KnowNamespacePrefixes.Uml, "classifierBehavior"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "collaborationRole"):
                                TryCollectMultiValueReferencePropertyIdentifiers(xmlReader, poco, "collaborationRole");
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "role"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of Collaboration at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"CollaborationReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: CollaborationReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
