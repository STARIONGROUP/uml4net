// -------------------------------------------------------------------------------------------------
// <copyright file="MessageOccurrenceSpecificationReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    public class MessageOccurrenceSpecificationReader : XmiElementReader<IMessageOccurrenceSpecification>, IXmiElementReader<IMessageOccurrenceSpecification>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<MessageOccurrenceSpecificationReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="MessageOccurrenceSpecificationReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public MessageOccurrenceSpecificationReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<MessageOccurrenceSpecificationReader>.Instance : loggerFactory.CreateLogger<MessageOccurrenceSpecificationReader>();
        }

        /// <summary>
        /// Reads the <see cref="IMessageOccurrenceSpecification"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IMessageOccurrenceSpecification"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IMessageOccurrenceSpecification"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IMessageOccurrenceSpecification"/>
        /// </returns>
        public override IMessageOccurrenceSpecification Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            {
                this.logger.LogTrace("reading MessageOccurrenceSpecification at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.ResolveQualifiedName(xmlReader.GetXmiAttribute("type"), this.NameSpaceResolver);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:MessageOccurrenceSpecification")
                {
                    throw new XmlException($"The XmiType should be 'uml:MessageOccurrenceSpecification' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:MessageOccurrenceSpecification";
                }
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: MessageOccurrenceSpecification [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            case (KnowNamespacePrefixes.Uml, "covered"):
                                CollectSingleValueReferencePropertyIdentifier(xmlReader, poco, "covered");
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "message"):
                                CollectSingleValueReferencePropertyIdentifier(xmlReader, poco, "message");
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "toAfter"):
                                TryCollectMultiValueReferencePropertyIdentifiers(xmlReader, poco, "toAfter");
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "qualifiedName"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of MessageOccurrenceSpecification at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"MessageOccurrenceSpecificationReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: MessageOccurrenceSpecificationReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
