// -------------------------------------------------------------------------------------------------
// <copyright file="DependencyReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="DependencyReader"/> is to read an instance of <see cref="IDependency"/>
    ...
    public class DependencyReader : XmiElementReader<IDependency>, IXmiElementReader<IDependency>
    {
        ...
        /// </param>
        public DependencyReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
        ...
        /// <summary>
        /// Reads the <see cref="IDependency"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IDependency"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IDependency"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IDependency"/>
        /// </returns>
        public override IDependency Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                {
                    throw new XmlException($"The XmiType should be 'uml:Dependency' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:Dependency";
                }
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: Dependency [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "target"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of Dependency at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
