...
namespace uml4net.xmi.Readers
{
    ...
    public class CombinedFragmentReader : XmiElementReader<ICombinedFragment>, IXmiElementReader<ICombinedFragment>
    {
        ...
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<CombinedFragmentReader> logger;
        ...
        /// </param>
        public CombinedFragmentReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<CombinedFragmentReader>.Instance : loggerFactory.CreateLogger<CombinedFragmentReader>();
        }
        ...
        /// The name of the document that contains the <see cref="ICombinedFragment"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="ICombinedFragment"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="ICombinedFragment"/>
        /// </returns>
        public override ICombinedFragment Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: CombinedFragment [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "qualifiedName"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of CombinedFragment at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    this.logger.LogWarning("Not Supported: CombinedFragmentReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
