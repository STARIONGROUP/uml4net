        ...
        public override IActivityPartition Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: ActivityPartition [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "isDimension"):
                                var isDimensionValue = xmlReader.ReadElementContentAsStringInPlace();
                            ...
                            case (KnowNamespacePrefixes.Uml, "isExternal"):
                                var isExternalValue = xmlReader.ReadElementContentAsStringInPlace();
                            ...
                            case (KnowNamespacePrefixes.Uml, "subpartition"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "subpartition", poco.Subpartition.Count))
                                {
                            ...
                            case (KnowNamespacePrefixes.Uml, "superGroup"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of ActivityPartition at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
