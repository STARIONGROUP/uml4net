// -------------------------------------------------------------------------------------------------
// <copyright file="ComponentReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="ComponentReader"/> is to read an instance of <see cref="IComponent"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class ComponentReader : XmiElementReader<IComponent>, IXmiElementReader<IComponent>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<ComponentReader> logger;
        ...
        /// </param>
        public ComponentReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<ComponentReader>.Instance : loggerFactory.CreateLogger<ComponentReader>();
        }

        /// <summary>
        /// Reads the <see cref="IComponent"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IComponent"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IComponent"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IComponent"/>
        /// </returns>
        public override IComponent Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IComponent poco = new Component();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading Component at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.ResolveQualifiedName(xmlReader.GetXmiAttribute("type"), this.NameSpaceResolver);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:Component")
                {
                    throw new XmlException($"The XmiType should be 'uml:Component' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:Component";
                }
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: Component [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                ...
                var isIndirectlyInstantiatedXmlAttribute = xmlReader.GetAttribute("isIndirectlyInstantiated") ?? xmlReader.GetAttribute("isIndirectlyInstantiated", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(isIndirectlyInstantiatedXmlAttribute))
                {
                    poco.IsIndirectlyInstantiated = XmlConvert.ToBoolean(isIndirectlyInstantiatedXmlAttribute);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "isFinalSpecialization"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "isIndirectlyInstantiated"):
                                var isIndirectlyInstantiatedValue = xmlReader.ReadElementContentAsStringInPlace();

                                if (!string.IsNullOrWhiteSpace(isIndirectlyInstantiatedValue))
                                {
                                    poco.IsIndirectlyInstantiated = XmlConvert.ToBoolean(isIndirectlyInstantiatedValue);
                                }
                            ...
                            case (KnowNamespacePrefixes.Uml, "packagedElement"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "packagedElement", poco.PackagedElement.Count))
                                {
                            ...
                            case (KnowNamespacePrefixes.Uml, "realization"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "realization", poco.Realization.Count))
                                {
                            ...
                            case (KnowNamespacePrefixes.Uml, "provided"):
                            ...
                            case (KnowNamespacePrefixes.Uml, "required"):
                            ...
                            case (KnowNamespacePrefixes.Uml, "superClass"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of Component at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"ComponentReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: ComponentReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
