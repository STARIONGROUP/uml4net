...
namespace uml4net.xmi.Readers
{
    ...
    public class ComponentReader : XmiElementReader<IComponent>, IXmiElementReader<IComponent>
    {
        ...
        /// </param>
        public ComponentReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, ILoggerFactory loggerFactory)
        ...
        public override IComponent Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                var isIndirectlyInstantiatedXmlAttribute = xmlReader.GetAttribute("isIndirectlyInstantiated");
                if (!string.IsNullOrEmpty(isIndirectlyInstantiatedXmlAttribute))
                {
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "isFinalSpecialization":
                                ...
                                break;
                            case "isIndirectlyInstantiated":
                                var isIndirectlyInstantiatedValue = xmlReader.ReadElementContentAsString();
                                if (!string.IsNullOrEmpty(isIndirectlyInstantiatedValue))
                                {
                            ...
                            case "powertypeExtent":
                                ...
                                break;
                            case "realization":
                                var realizationValue = (IComponentRealization)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.LoggerFactory, "uml:ComponentRealization");
                            ...
                            case "visibility":
                                ...
                                break;
                            default:
                                throw new NotSupportedException($"ComponentReader: {xmlReader.LocalName} at line:position {defaultLineInfo.LineNumber}:{defaultLineInfo.LinePosition}");
                        }
                    }
                }
            }
