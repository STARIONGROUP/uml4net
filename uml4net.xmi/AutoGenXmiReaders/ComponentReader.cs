...
namespace uml4net.xmi.Readers.StructuredClassifiers
{
    ...
    public class ComponentReader : XmiElementReader<IComponent>, IXmiElementReader<IComponent>
    {
        ...
        /// </param>
        public ComponentReader(IXmiReaderCache cache, ILoggerFactory loggerFactory)
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "isIndirectlyInstantiated":
                                var isIndirectlyInstantiatedXmlElement = xmlReader.ReadElementContentAsString();
                                if (!string.IsNullOrEmpty(isIndirectlyInstantiatedXmlElement))
                                {
                                    poco.IsIndirectlyInstantiated = bool.Parse(isIndirectlyInstantiatedXmlElement);
                                }
                                break;
                            case "isLeaf":
                            ...
                            case "powertypeExtent":
                                ...
                                break;
                            case "realization":
                                var realization = (IComponentRealization)this.xmiElementReaderFacade.QueryXmiElement(xmlReader, this.Cache, this.LoggerFactory, "uml:ComponentRealization");
                                poco.Realization.Add(realization);
                                break;
