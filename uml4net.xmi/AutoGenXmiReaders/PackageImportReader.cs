...
namespace uml4net.xmi.Readers
{
    ...
    public class PackageImportReader : XmiElementReader<IPackageImport>, IXmiElementReader<IPackageImport>
    {
        ...
        /// </param>
        public PackageImportReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<PackageImportReader>.Instance : loggerFactory.CreateLogger<PackageImportReader>();
        }

        /// <summary>
        /// Reads the <see cref="IPackageImport"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IPackageImport"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IPackageImport"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IPackageImport"/>
        /// </returns>
        public override IPackageImport Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IPackageImport poco = new PackageImport();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading PackageImport at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: PackageImport [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }

                var importedPackageXmlAttribute = xmlReader.GetAttribute("importedPackage") ?? xmlReader.GetAttribute("importedPackage", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(importedPackageXmlAttribute))
                {
                    poco.SingleValueReferencePropertyIdentifiers.Add("importedPackage", importedPackageXmlAttribute);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "target"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of PackageImport at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"PackageImportReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: PackageImportReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
