// -------------------------------------------------------------------------------------------------
// <copyright file="FunctionBehaviorReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="FunctionBehaviorReader"/> is to read an instance of <see cref="IFunctionBehavior"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class FunctionBehaviorReader : XmiElementReader<IFunctionBehavior>, IXmiElementReader<IFunctionBehavior>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<FunctionBehaviorReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="FunctionBehaviorReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public FunctionBehaviorReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<FunctionBehaviorReader>.Instance : loggerFactory.CreateLogger<FunctionBehaviorReader>();
        }

        /// <summary>
        /// Reads the <see cref="IFunctionBehavior"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IFunctionBehavior"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IFunctionBehavior"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IFunctionBehavior"/>
        /// </returns>
        public override IFunctionBehavior Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IFunctionBehavior poco = new FunctionBehavior();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading FunctionBehavior at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.ResolveQualifiedName(xmlReader.GetXmiAttribute("type"), this.NameSpaceResolver);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:FunctionBehavior")
                {
                    throw new XmlException($"The XmiType should be 'uml:FunctionBehavior' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:FunctionBehavior";
                }
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: FunctionBehavior [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                ...
                {
                    var bodyXmlAttributeValues = bodyXmlAttribute.Split(this.XmiReaderSettings.ValueSeparator);
                    foreach (var bodyXmlAttributeValue in bodyXmlAttributeValues)
                    {
                        poco.Body.Add(bodyXmlAttributeValue);
                    }
                }
                ...
                if (!string.IsNullOrWhiteSpace(languageXmlAttribute))
                {
                    var languageXmlAttributeValues = languageXmlAttribute.Split(this.XmiReaderSettings.ValueSeparator);
                    foreach (var languageXmlAttributeValue in languageXmlAttributeValues)
                    {
                        poco.Language.Add(languageXmlAttributeValue);
                    }
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "superClass"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of FunctionBehavior at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"FunctionBehaviorReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: FunctionBehaviorReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
