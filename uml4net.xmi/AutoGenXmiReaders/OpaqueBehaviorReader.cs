...
namespace uml4net.xmi.Readers
{
    ...
    public class OpaqueBehaviorReader : XmiElementReader<IOpaqueBehavior>, IXmiElementReader<IOpaqueBehavior>
    {
        ...
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<OpaqueBehaviorReader> logger;
        ...
        /// </param>
        public OpaqueBehaviorReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<OpaqueBehaviorReader>.Instance : loggerFactory.CreateLogger<OpaqueBehaviorReader>();
        }
        ...
        /// The name of the document that contains the <see cref="IOpaqueBehavior"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IOpaqueBehavior"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IOpaqueBehavior"/>
        /// </returns>
        public override IOpaqueBehavior Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            {
                this.logger.LogTrace("reading OpaqueBehavior at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: OpaqueBehavior [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "superClass"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of OpaqueBehavior at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"OpaqueBehaviorReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: OpaqueBehaviorReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
