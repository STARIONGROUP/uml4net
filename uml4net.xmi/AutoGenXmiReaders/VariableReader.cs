...
namespace uml4net.xmi.Readers
{
    ...
    public class VariableReader : XmiElementReader<IVariable>, IXmiElementReader<IVariable>
    {
        ...
        /// </param>
        public VariableReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
        ...
        public override IVariable Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: Variable [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                ...
                if (!string.IsNullOrWhiteSpace(activityScopeXmlAttribute))
                {
                ...
                if (!string.IsNullOrWhiteSpace(scopeXmlAttribute))
                {
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            case (KnowNamespacePrefixes.Uml, "activityScope"):
                                CollectSingleValueReferencePropertyIdentifier(xmlReader, poco, "activityScope");
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "scope"):
                                CollectSingleValueReferencePropertyIdentifier(xmlReader, poco, "scope");
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "upper"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of Variable at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
