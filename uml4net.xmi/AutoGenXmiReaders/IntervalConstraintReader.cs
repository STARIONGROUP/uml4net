// -------------------------------------------------------------------------------------------------
// <copyright file="IntervalConstraintReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="IntervalConstraintReader"/> is to read an instance of <see cref="IIntervalConstraint"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class IntervalConstraintReader : XmiElementReader<IIntervalConstraint>, IXmiElementReader<IIntervalConstraint>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<IntervalConstraintReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="IntervalConstraintReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public IntervalConstraintReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<IntervalConstraintReader>.Instance : loggerFactory.CreateLogger<IntervalConstraintReader>();
        }

        /// <summary>
        /// Reads the <see cref="IIntervalConstraint"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IIntervalConstraint"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IIntervalConstraint"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IIntervalConstraint"/>
        /// </returns>
        public override IIntervalConstraint Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IIntervalConstraint poco = new IntervalConstraint();
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                {
                    throw new XmlException($"The XmiType should be 'uml:IntervalConstraint' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:IntervalConstraint";
                }
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: IntervalConstraint [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "specification"):
                                ...
                                {
                                    var specificationValue = (IInterval)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.ExtenderReaderRegistry, this.LoggerFactory, "uml:Interval");
                            ...
                            case (KnowNamespacePrefixes.Uml, "qualifiedName"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of IntervalConstraint at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"IntervalConstraintReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: IntervalConstraintReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
