...
namespace uml4net.xmi.Readers
{
    ...
    public class ProtocolConformanceReader : XmiElementReader<IProtocolConformance>, IXmiElementReader<IProtocolConformance>
    {
        ...
        /// </param>
        public ProtocolConformanceReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
        ...
        public override IProtocolConformance Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: ProtocolConformance [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                ...
                while (xmlReader.Read())
                {
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        {
                            case (KnowNamespacePrefixes.Uml, "generalMachine"):
                                CollectSingleValueReferencePropertyIdentifier(xmlReader, poco, "generalMachine");
                                break;
                            case (KnowNamespacePrefixes.Uml, "ownedComment"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "specificMachine"):
                                CollectSingleValueReferencePropertyIdentifier(xmlReader, poco, "specificMachine");
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "target"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of ProtocolConformance at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
