// -------------------------------------------------------------------------------------------------
// <copyright file="InstanceSpecificationReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="InstanceSpecificationReader"/> is to read an instance of <see cref="IInstanceSpecification"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class InstanceSpecificationReader : XmiElementReader<IInstanceSpecification>, IXmiElementReader<IInstanceSpecification>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<InstanceSpecificationReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="InstanceSpecificationReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public InstanceSpecificationReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<InstanceSpecificationReader>.Instance : loggerFactory.CreateLogger<InstanceSpecificationReader>();
        }

        /// <summary>
        /// Reads the <see cref="IInstanceSpecification"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IInstanceSpecification"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IInstanceSpecification"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IInstanceSpecification"/>
        /// </returns>
        public override IInstanceSpecification Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IInstanceSpecification poco = new InstanceSpecification();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading InstanceSpecification at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.ResolveQualifiedName(xmlReader.GetXmiAttribute("type"), this.NameSpaceResolver);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:InstanceSpecification")
                {
                    throw new XmlException($"The XmiType should be 'uml:InstanceSpecification' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:InstanceSpecification";
                }
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: InstanceSpecification [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                ...
                {
                    var classifierXmlAttributeValues = classifierXmlAttribute.Split(SplitMultiReference, StringSplitOptions.RemoveEmptyEntries).ToList();
                    poco.MultiValueReferencePropertyIdentifiers.Add("classifier", classifierXmlAttributeValues);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            case (KnowNamespacePrefixes.Uml, "classifier"):
                                TryCollectMultiValueReferencePropertyIdentifiers(xmlReader, poco, "classifier");
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "owningTemplateParameter"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "slot"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "slot", poco.Slot.Count))
                                {
                                    var slotValue = (ISlot)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.ExtenderReaderRegistry, this.LoggerFactory, "uml:Slot");
                            ...
                            case (KnowNamespacePrefixes.Uml, "qualifiedName"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of InstanceSpecification at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"InstanceSpecificationReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: InstanceSpecificationReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
