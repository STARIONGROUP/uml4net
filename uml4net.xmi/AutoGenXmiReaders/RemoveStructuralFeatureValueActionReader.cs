...
namespace uml4net.xmi.Readers
{
    ...
    public class RemoveStructuralFeatureValueActionReader : XmiElementReader<IRemoveStructuralFeatureValueAction>, IXmiElementReader<IRemoveStructuralFeatureValueAction>
    {
        ...
        /// </param>
        public RemoveStructuralFeatureValueActionReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
        ...
        public override IRemoveStructuralFeatureValueAction Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: RemoveStructuralFeatureValueAction [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                ...
                {
                    poco.IsRemoveDuplicates = XmlConvert.ToBoolean(isRemoveDuplicatesXmlAttribute);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "isRemoveDuplicates"):
                                var isRemoveDuplicatesValue = xmlReader.ReadElementContentAsStringInPlace();

                                if (!string.IsNullOrWhiteSpace(isRemoveDuplicatesValue))
                                {
                                    poco.IsRemoveDuplicates = XmlConvert.ToBoolean(isRemoveDuplicatesValue);
                                }
                            ...
                            case (KnowNamespacePrefixes.Uml, "structuralFeature"):
                                CollectSingleValueReferencePropertyIdentifier(xmlReader, poco, "structuralFeature");
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "redefinitionContext"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of RemoveStructuralFeatureValueAction at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
