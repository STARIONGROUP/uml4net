// -------------------------------------------------------------------------------------------------
// <copyright file="ExtendReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="ExtendReader"/> is to read an instance of <see cref="IExtend"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class ExtendReader : XmiElementReader<IExtend>, IXmiElementReader<IExtend>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<ExtendReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="ExtendReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public ExtendReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<ExtendReader>.Instance : loggerFactory.CreateLogger<ExtendReader>();
        }

        /// <summary>
        /// Reads the <see cref="IExtend"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IExtend"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IExtend"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IExtend"/>
        /// </returns>
        public override IExtend Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IExtend poco = new Extend();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading Extend at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.ResolveQualifiedName(xmlReader.GetXmiAttribute("type"), this.NameSpaceResolver);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:Extend")
                {
                    throw new XmlException($"The XmiType should be 'uml:Extend' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:Extend";
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: Extend [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }

                var extendedCaseXmlAttribute = xmlReader.GetAttribute("extendedCase") ?? xmlReader.GetAttribute("extendedCase", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(extendedCaseXmlAttribute))
                {
                    poco.SingleValueReferencePropertyIdentifiers.Add("extendedCase", extendedCaseXmlAttribute);
                }

                var extensionXmlAttribute = xmlReader.GetAttribute("extension") ?? xmlReader.GetAttribute("extension", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(extensionXmlAttribute))
                {
                    poco.SingleValueReferencePropertyIdentifiers.Add("extension", extensionXmlAttribute);
                }

                var extensionLocationXmlAttribute = xmlReader.GetAttribute("extensionLocation") ?? xmlReader.GetAttribute("extensionLocation", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(extensionLocationXmlAttribute))
                {
                    ...
                    poco.MultiValueReferencePropertyIdentifiers.Add("extensionLocation", extensionLocationXmlAttributeValues);
                }
                ...
                while (xmlReader.Read())
                {
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        {
                            case (KnowNamespacePrefixes.Uml, "condition"):
                            ...
                            case (KnowNamespacePrefixes.Uml, "target"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of Extend at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"ExtendReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: ExtendReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
