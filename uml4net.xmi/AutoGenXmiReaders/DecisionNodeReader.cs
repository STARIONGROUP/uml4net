...
namespace uml4net.xmi.Readers
{
    ...
    public class DecisionNodeReader : XmiElementReader<IDecisionNode>, IXmiElementReader<IDecisionNode>
    {
        ...
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<DecisionNodeReader> logger;
        ...
        /// </param>
        public DecisionNodeReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<DecisionNodeReader>.Instance : loggerFactory.CreateLogger<DecisionNodeReader>();
        }
        ...
        /// The name of the document that contains the <see cref="IDecisionNode"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IDecisionNode"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IDecisionNode"/>
        /// </returns>
        public override IDecisionNode Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            {
                this.logger.LogTrace("reading DecisionNode at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: DecisionNode [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                ...
                var decisionInputXmlAttribute = xmlReader.GetAttribute("decisionInput") ?? xmlReader.GetAttribute("decisionInput", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(decisionInputXmlAttribute))
                {
                    poco.SingleValueReferencePropertyIdentifiers.Add("decisionInput", decisionInputXmlAttribute);
                }

                var decisionInputFlowXmlAttribute = xmlReader.GetAttribute("decisionInputFlow") ?? xmlReader.GetAttribute("decisionInputFlow", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(decisionInputFlowXmlAttribute))
                {
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            case (KnowNamespacePrefixes.Uml, "activity"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "decisionInput"):
                                CollectSingleValueReferencePropertyIdentifier(xmlReader, poco, "decisionInput");
                                break;
                            case (KnowNamespacePrefixes.Uml, "decisionInputFlow"):
                                CollectSingleValueReferencePropertyIdentifier(xmlReader, poco, "decisionInputFlow");
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "redefinitionContext"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of DecisionNode at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"DecisionNodeReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: DecisionNodeReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
