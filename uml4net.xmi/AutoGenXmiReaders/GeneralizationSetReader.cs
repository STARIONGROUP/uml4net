...
namespace uml4net.xmi.Readers
{
    ...
    public class GeneralizationSetReader : XmiElementReader<IGeneralizationSet>, IXmiElementReader<IGeneralizationSet>
    {
        ...
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<GeneralizationSetReader> logger;
        ...
        /// </param>
        public GeneralizationSetReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<GeneralizationSetReader>.Instance : loggerFactory.CreateLogger<GeneralizationSetReader>();
        }
        ...
        /// The name of the document that contains the <see cref="IGeneralizationSet"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IGeneralizationSet"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IGeneralizationSet"/>
        /// </returns>
        public override IGeneralizationSet Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            {
                this.logger.LogTrace("reading GeneralizationSet at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: GeneralizationSet [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }

                var generalizationXmlAttribute = xmlReader.GetAttribute("generalization") ?? xmlReader.GetAttribute("generalization", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(generalizationXmlAttribute))
                {
                ...
                var isCoveringXmlAttribute = xmlReader.GetAttribute("isCovering") ?? xmlReader.GetAttribute("isCovering", this.NameSpaceResolver.UmlNameSpace);
                ...
                var powertypeXmlAttribute = xmlReader.GetAttribute("powertype") ?? xmlReader.GetAttribute("powertype", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(powertypeXmlAttribute))
                {
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "isCovering"):
                                var isCoveringValue = xmlReader.ReadElementContentAsStringInPlace();
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "isDisjoint"):
                                var isDisjointValue = xmlReader.ReadElementContentAsStringInPlace();
                            ...
                            case (KnowNamespacePrefixes.Uml, "qualifiedName"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of GeneralizationSet at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"GeneralizationSetReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: GeneralizationSetReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
