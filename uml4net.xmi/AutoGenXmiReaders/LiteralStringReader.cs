...
namespace uml4net.xmi.Readers
{
    ...
    public class LiteralStringReader : XmiElementReader<ILiteralString>, IXmiElementReader<ILiteralString>
    {
        ...
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<LiteralStringReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="LiteralStringReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public LiteralStringReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<LiteralStringReader>.Instance : loggerFactory.CreateLogger<LiteralStringReader>();
        }
        ...
        /// The name of the document that contains the <see cref="ILiteralString"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="ILiteralString"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="ILiteralString"/>
        /// </returns>
        public override ILiteralString Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            {
                this.logger.LogTrace("reading LiteralString at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: LiteralString [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "value"):
                                poco.Value = xmlReader.ReadElementContentAsStringInPlace();
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "qualifiedName"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of LiteralString at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"LiteralStringReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: LiteralStringReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
