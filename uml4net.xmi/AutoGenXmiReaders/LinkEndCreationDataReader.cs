...
namespace uml4net.xmi.Readers
{
    ...
    public class LinkEndCreationDataReader : XmiElementReader<ILinkEndCreationData>, IXmiElementReader<ILinkEndCreationData>
    {
        ...
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<LinkEndCreationDataReader> logger;
        ...
        /// </param>
        public LinkEndCreationDataReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<LinkEndCreationDataReader>.Instance : loggerFactory.CreateLogger<LinkEndCreationDataReader>();
        }
        ...
        /// The name of the document that contains the <see cref="ILinkEndCreationData"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="ILinkEndCreationData"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="ILinkEndCreationData"/>
        /// </returns>
        public override ILinkEndCreationData Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            {
                this.logger.LogTrace("reading LinkEndCreationData at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: LinkEndCreationData [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "owner"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of LinkEndCreationData at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"LinkEndCreationDataReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: LinkEndCreationDataReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
