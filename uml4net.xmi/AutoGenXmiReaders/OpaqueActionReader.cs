// -------------------------------------------------------------------------------------------------
// <copyright file="OpaqueActionReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="OpaqueActionReader"/> is to read an instance of <see cref="IOpaqueAction"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class OpaqueActionReader : XmiElementReader<IOpaqueAction>, IXmiElementReader<IOpaqueAction>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<OpaqueActionReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="OpaqueActionReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public OpaqueActionReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<OpaqueActionReader>.Instance : loggerFactory.CreateLogger<OpaqueActionReader>();
        }

        /// <summary>
        /// Reads the <see cref="IOpaqueAction"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IOpaqueAction"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IOpaqueAction"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IOpaqueAction"/>
        /// </returns>
        public override IOpaqueAction Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IOpaqueAction poco = new OpaqueAction();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading OpaqueAction at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.ResolveQualifiedName(xmlReader.GetXmiAttribute("type"), this.NameSpaceResolver);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:OpaqueAction")
                {
                    throw new XmlException($"The XmiType should be 'uml:OpaqueAction' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:OpaqueAction";
                }
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: OpaqueAction [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "inPartition"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "inputValue"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "inputValue", poco.InputValue.Count))
                                {
                                    var inputValueValue = (IInputPin)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.ExtenderReaderRegistry, this.LoggerFactory, "uml:InputPin");
                                    poco.InputValue.Add(inputValueValue);
                                }
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "outgoing"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "outputValue"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "outputValue", poco.OutputValue.Count))
                                {
                                    var outputValueValue = (IOutputPin)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.ExtenderReaderRegistry, this.LoggerFactory, "uml:OutputPin");
                                    poco.OutputValue.Add(outputValueValue);
                                }
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "redefinitionContext"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of OpaqueAction at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"OpaqueActionReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: OpaqueActionReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
