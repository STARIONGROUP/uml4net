...
namespace uml4net.xmi.Readers
{
    ...
    public class ConnectionPointReferenceReader : XmiElementReader<IConnectionPointReference>, IXmiElementReader<IConnectionPointReference>
    {
        ...
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<ConnectionPointReferenceReader> logger;
        ...
        /// </param>
        public ConnectionPointReferenceReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<ConnectionPointReferenceReader>.Instance : loggerFactory.CreateLogger<ConnectionPointReferenceReader>();
        }
        ...
        /// The name of the document that contains the <see cref="IConnectionPointReference"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IConnectionPointReference"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IConnectionPointReference"/>
        /// </returns>
        public override IConnectionPointReference Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            {
                this.logger.LogTrace("reading ConnectionPointReference at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: ConnectionPointReference [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                ...
                var entryXmlAttribute = xmlReader.GetAttribute("entry") ?? xmlReader.GetAttribute("entry", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(entryXmlAttribute))
                {
                ...
                var exitXmlAttribute = xmlReader.GetAttribute("exit") ?? xmlReader.GetAttribute("exit", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(exitXmlAttribute))
                {
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "redefinitionContext"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of ConnectionPointReference at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"ConnectionPointReferenceReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: ConnectionPointReferenceReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
