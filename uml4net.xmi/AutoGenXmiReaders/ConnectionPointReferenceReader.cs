...
namespace uml4net.xmi.Readers.StateMachines
{
    ...
    public class ConnectionPointReferenceReader : XmiElementReader<IConnectionPointReference>, IXmiElementReader<IConnectionPointReference>
    {
        ...
        /// </param>
        public ConnectionPointReferenceReader(IXmiReaderCache cache, ILoggerFactory loggerFactory)
        ...
        public override IConnectionPointReference Read(XmlReader xmlReader)
        {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmlReader.LocalName)
                        {
                            case "container":
                                ...
                                break;
                            case "entry":
                                this.CollectMultiValueReferencePropertyIdentifiers(xmlReader, entry, "entry");
                                break;
                            case "exit":
                                this.CollectMultiValueReferencePropertyIdentifiers(xmlReader, exit, "exit");
                                break;
                            case "isLeaf":
                ...
                if (entry.Count > 0)
                {
                    poco.MultiValueReferencePropertyIdentifiers.Add("entry", entry);
                }

                if (exit.Count > 0)
                {
                    poco.MultiValueReferencePropertyIdentifiers.Add("exit", exit);
                }
