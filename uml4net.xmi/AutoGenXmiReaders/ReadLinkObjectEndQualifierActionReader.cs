// -------------------------------------------------------------------------------------------------
// <copyright file="ReadLinkObjectEndQualifierActionReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="ReadLinkObjectEndQualifierActionReader"/> is to read an instance of <see cref="IReadLinkObjectEndQualifierAction"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class ReadLinkObjectEndQualifierActionReader : XmiElementReader<IReadLinkObjectEndQualifierAction>, IXmiElementReader<IReadLinkObjectEndQualifierAction>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<ReadLinkObjectEndQualifierActionReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="ReadLinkObjectEndQualifierActionReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public ReadLinkObjectEndQualifierActionReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<ReadLinkObjectEndQualifierActionReader>.Instance : loggerFactory.CreateLogger<ReadLinkObjectEndQualifierActionReader>();
        }

        /// <summary>
        /// Reads the <see cref="IReadLinkObjectEndQualifierAction"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="IReadLinkObjectEndQualifierAction"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="IReadLinkObjectEndQualifierAction"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="IReadLinkObjectEndQualifierAction"/>
        /// </returns>
        public override IReadLinkObjectEndQualifierAction Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            IReadLinkObjectEndQualifierAction poco = new ReadLinkObjectEndQualifierAction();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading ReadLinkObjectEndQualifierAction at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.GetAttribute("type", this.NameSpaceResolver.XmiNameSpace);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:ReadLinkObjectEndQualifierAction")
                {
                    throw new XmlException($"The XmiType should be 'uml:ReadLinkObjectEndQualifierAction' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:ReadLinkObjectEndQualifierAction";
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "ReadLinkObjectEndQualifierAction", poco.XmiId);
                }
                ...
                var qualifierXmlAttribute = xmlReader.GetAttribute("qualifier") ?? xmlReader.GetAttribute("qualifier", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(qualifierXmlAttribute))
                {
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"ReadLinkObjectEndQualifierActionReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: ReadLinkObjectEndQualifierActionReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                                }
