                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmlReader.LocalName)
                        {
                            case "activity":
                                ...
                                break;
                            case "clause":
                                var clause = (IClause)this.xmiElementReaderFacade.QueryXmiElement(xmlReader, this.Cache, this.LoggerFactory, "uml:Clause");
                                poco.Clause.Add(clause);
                                break;
                            case "edge":
                            ...
                            case "inStructuredNode":
                                ...
                                break;
                            case "isAssured":
                                var isAssuredXmlElement = xmlReader.ReadElementContentAsString();
                                if (!string.IsNullOrEmpty(isAssuredXmlElement))
                                {
                                    poco.IsAssured = bool.Parse(isAssuredXmlElement);
                                }
                                break;
                            case "isDeterminate":
                                var isDeterminateXmlElement = xmlReader.ReadElementContentAsString();
                                if (!string.IsNullOrEmpty(isDeterminateXmlElement))
                                {
                                    poco.IsDeterminate = bool.Parse(isDeterminateXmlElement);
                                }
                                break;
                            case "isLeaf":
