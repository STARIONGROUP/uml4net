...
namespace uml4net.xmi.Readers
{
    ...
    public class ConditionalNodeReader : XmiElementReader<IConditionalNode>, IXmiElementReader<IConditionalNode>
    {
        ...
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<ConditionalNodeReader> logger;
        ...
        /// </param>
        public ConditionalNodeReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<ConditionalNodeReader>.Instance : loggerFactory.CreateLogger<ConditionalNodeReader>();
        }
        ...
        public override IConditionalNode Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            ...
            {
                this.logger.LogTrace("reading ConditionalNode at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                var isAssuredXmlAttribute = xmlReader.GetAttribute("isAssured") ?? xmlReader.GetAttribute("isAssured", this.NameSpaceResolver.UmlNameSpace);
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "clause"):
                                var clauseValue = (IClause)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.LoggerFactory, "uml:Clause");
                            ...
                            case (KnowNamespacePrefixes.Uml, "isAssured"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "isDeterminate"):
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"ConditionalNodeReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: ConditionalNodeReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                                }
