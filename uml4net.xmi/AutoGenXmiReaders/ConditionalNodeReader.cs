...
namespace uml4net.xmi.Readers
{
    ...
    public class ConditionalNodeReader : XmiElementReader<IConditionalNode>, IXmiElementReader<IConditionalNode>
    {
        ...
        /// </param>
        public ConditionalNodeReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
        ...
        public override IConditionalNode Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: ConditionalNode [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "clause"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "clause", poco.Clause.Count))
                                {
                            ...
                            case (KnowNamespacePrefixes.Uml, "isAssured"):
                                var isAssuredValue = xmlReader.ReadElementContentAsStringInPlace();
                            ...
                            case (KnowNamespacePrefixes.Uml, "isDeterminate"):
                                var isDeterminateValue = xmlReader.ReadElementContentAsStringInPlace();
                            ...
                            case (KnowNamespacePrefixes.Uml, "superGroup"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of ConditionalNode at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
