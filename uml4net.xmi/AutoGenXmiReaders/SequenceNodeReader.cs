// -------------------------------------------------------------------------------------------------
// <copyright file="SequenceNodeReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="SequenceNodeReader"/> is to read an instance of <see cref="ISequenceNode"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class SequenceNodeReader : XmiElementReader<ISequenceNode>, IXmiElementReader<ISequenceNode>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<SequenceNodeReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="SequenceNodeReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public SequenceNodeReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<SequenceNodeReader>.Instance : loggerFactory.CreateLogger<SequenceNodeReader>();
        }

        /// <summary>
        /// Reads the <see cref="ISequenceNode"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="ISequenceNode"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="ISequenceNode"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="ISequenceNode"/>
        /// </returns>
        public override ISequenceNode Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            ISequenceNode poco = new SequenceNode();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading SequenceNode at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.GetAttribute("type", this.NameSpaceResolver.XmiNameSpace);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:SequenceNode")
                {
                    throw new XmlException($"The XmiType should be 'uml:SequenceNode' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:SequenceNode";
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "SequenceNode", poco.XmiId);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "elementImport"):
                                ...
                                break;
                            case (KnowNamespacePrefixes.Uml, "executableNode"):
                                var executableNodeValue = (IExecutableNode)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.LoggerFactory);
                                poco.ExecutableNode.Add(executableNodeValue);
                                break;
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"SequenceNodeReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: SequenceNodeReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                                }
