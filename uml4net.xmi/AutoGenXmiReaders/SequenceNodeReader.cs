...
namespace uml4net.xmi.Readers
{
    ...
    public class SequenceNodeReader : XmiElementReader<ISequenceNode>, IXmiElementReader<ISequenceNode>
    {
        ...
        /// </param>
        public SequenceNodeReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
        ...
        public override ISequenceNode Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: SequenceNode [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "executableNode"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "executableNode", poco.ExecutableNode.Count))
                                {
                                    var executableNodeValue = (IExecutableNode)this.XmiElementReaderFacade.QueryXmiElement(xmlReader, documentName, namespaceUri, this.Cache, this.XmiReaderSettings, this.NameSpaceResolver, this.ExtenderReaderRegistry, this.LoggerFactory);
                            ...
                            case (KnowNamespacePrefixes.Uml, "mustIsolate"):
                                var mustIsolateValue = xmlReader.ReadElementContentAsStringInPlace();
                            ...
                            case (KnowNamespacePrefixes.Uml, "structuredNodeInput"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "structuredNodeInput", poco.StructuredNodeInput.Count))
                                {
                            ...
                            case (KnowNamespacePrefixes.Uml, "structuredNodeOutput"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "structuredNodeOutput", poco.StructuredNodeOutput.Count))
                                {
                            ...
                            case (KnowNamespacePrefixes.Uml, "superGroup"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of SequenceNode at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
