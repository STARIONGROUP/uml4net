// -------------------------------------------------------------------------------------------------
// <copyright file="CreateObjectActionReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="CreateObjectActionReader"/> is to read an instance of <see cref="ICreateObjectAction"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class CreateObjectActionReader : XmiElementReader<ICreateObjectAction>, IXmiElementReader<ICreateObjectAction>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<CreateObjectActionReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="CreateObjectActionReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public CreateObjectActionReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<CreateObjectActionReader>.Instance : loggerFactory.CreateLogger<CreateObjectActionReader>();
        }

        /// <summary>
        /// Reads the <see cref="ICreateObjectAction"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="ICreateObjectAction"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="ICreateObjectAction"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="ICreateObjectAction"/>
        /// </returns>
        public override ICreateObjectAction Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            ICreateObjectAction poco = new CreateObjectAction();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading CreateObjectAction at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.GetAttribute("type", this.NameSpaceResolver.XmiNameSpace);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:CreateObjectAction")
                {
                    throw new XmlException($"The XmiType should be 'uml:CreateObjectAction' while it is {xmiType}");
                }
                else
                {
                    xmiType = "uml:CreateObjectAction";
                }
                ...
                {
                    this.logger.LogCritical("Failed to add element type [{Poco}] with id [{Id}] as it was already in the Cache. The XMI document seems to have duplicate xmi:id values", "CreateObjectAction", poco.XmiId);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"CreateObjectActionReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: CreateObjectActionReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                                }
