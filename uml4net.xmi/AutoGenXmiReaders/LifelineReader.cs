// -------------------------------------------------------------------------------------------------
// <copyright file="LifelineReader.cs" company="Starion Group S.A.">
//
...
namespace uml4net.xmi.Readers
{
    ...
    /// <summary>
    /// The purpose of the <see cref="LifelineReader"/> is to read an instance of <see cref="ILifeline"/>
    /// from the XMI document
    /// </summary>
    [GeneratedCode("uml4net", "latest")]
    public class LifelineReader : XmiElementReader<ILifeline>, IXmiElementReader<ILifeline>
    {
        /// <summary>
        /// The (injected) logger
        /// </summary>
        private readonly ILogger<LifelineReader> logger;

        /// <summary>
        /// Initializes a new instance of the <see cref="LifelineReader"/> class.
        /// </summary>
        /// <param name="cache">
        ...
        /// </param>
        public LifelineReader(IXmiElementCache cache, IXmiElementReaderFacade xmiElementReaderFacade, IXmiReaderSettings xmiReaderSettings, INameSpaceResolver nameSpaceResolver, IExtenderReaderRegistry extenderReaderRegistry, ILoggerFactory loggerFactory)
            : base(cache, xmiElementReaderFacade, xmiReaderSettings, nameSpaceResolver, extenderReaderRegistry, loggerFactory)
        {
            this.logger = loggerFactory == null ? NullLogger<LifelineReader>.Instance : loggerFactory.CreateLogger<LifelineReader>();
        }

        /// <summary>
        /// Reads the <see cref="ILifeline"/> object from its XML representation
        /// </summary>
        /// <param name="xmlReader">
        /// an instance of <see cref="XmlReader"/>
        /// </param>
        /// <param name="documentName">
        /// The name of the document that contains the <see cref="ILifeline"/>
        /// </param>
        /// <param name="namespaceUri">
        /// the namespace that the <see cref="ILifeline"/> belongs to
        /// </param>
        /// <returns>
        /// an instance of <see cref="ILifeline"/>
        /// </returns>
        public override ILifeline Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            ILifeline poco = new Lifeline();

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.logger.LogTrace("reading Lifeline at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);

                var xmiType = xmlReader.ResolveQualifiedName(xmlReader.GetXmiAttribute("type"), this.NameSpaceResolver);

                if (!string.IsNullOrEmpty(xmiType) && xmiType != "uml:Lifeline")
                {
                    throw new XmlException($"The XmiType should be 'uml:Lifeline' while it is {xmiType}");
                }
                else
                {
                ...
                if (!this.Cache.TryAdd(poco))
                {
                    ...
                    this.logger.LogError("The xmi:id is not unique within the document, the element that was read first is kept: Lifeline [{XmiId}] property [xmi:id] at line:position {LineNumber}:{LinePosition}", poco.XmiId, xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                }

                var coveredByXmlAttribute = xmlReader.GetAttribute("coveredBy") ?? xmlReader.GetAttribute("coveredBy", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(coveredByXmlAttribute))
                {
                    var coveredByXmlAttributeValues = coveredByXmlAttribute.Split(SplitMultiReference, StringSplitOptions.RemoveEmptyEntries).ToList();
                    poco.MultiValueReferencePropertyIdentifiers.Add("coveredBy", coveredByXmlAttributeValues);
                }

                var decomposedAsXmlAttribute = xmlReader.GetAttribute("decomposedAs") ?? xmlReader.GetAttribute("decomposedAs", this.NameSpaceResolver.UmlNameSpace);

                if (!string.IsNullOrWhiteSpace(decomposedAsXmlAttribute))
                {
                    poco.SingleValueReferencePropertyIdentifiers.Add("decomposedAs", decomposedAsXmlAttribute);
                }
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        ...
                        switch (activePrefix, xmlReader.LocalName)
                        {
                            ...
                            case (KnowNamespacePrefixes.Uml, "selector"):
                                if (!TryCollectCompositeReferencePropertyIdentifier(xmlReader, poco, "selector", poco.Selector.Count))
                                {
                                    ...
                                    poco.Selector.Add(selectorValue);
                                }
                                break;
                            ...
                            case (KnowNamespacePrefixes.Uml, "qualifiedName"):
                                ...
                                this.logger.LogDebug("Ignoring the serialized derived property {LocalName} of Lifeline at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"LifelineReader: {xmlReader.LocalName} at line:position {xmlLineInfo.LineNumber}:{xmlLineInfo.LinePosition}");
                                }
                                else
                                {
                                    this.logger.LogWarning("Not Supported: LifelineReader: {LocalName} at line:position {LineNumber}:{LinePosition}", xmlReader.LocalName, xmlLineInfo.LineNumber, xmlLineInfo.LinePosition);
