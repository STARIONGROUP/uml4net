...
namespace uml4net.xmi.Readers.Values
{
    ...
    public class LiteralRealReader : XmiElementReader<ILiteralReal>, IXmiElementReader<ILiteralReal>
    {
        ...
        /// </param>
        public LiteralRealReader(IXmiReaderCache cache, ILoggerFactory loggerFactory)
        ...
        /// </returns>
        public override ILiteralReal Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                    ...
                    if (xmlReader.NodeType == XmlNodeType.Element)
                    {
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "type":
                                ...
                                break;
                            case "value":
                                var valueXmlElement = xmlReader.ReadElementContentAsString();
                                if (!string.IsNullOrEmpty(valueXmlElement))
                                {
                                    poco.Value = double.Parse(valueXmlElement);
                                }
                                break;
                            case "visibility":
