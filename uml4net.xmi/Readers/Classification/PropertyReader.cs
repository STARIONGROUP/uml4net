    ...
    public class PropertyReader : XmiElementReader<IProperty>, IXmiElementReader<IProperty>
    {
        ...
        private void ReadValueSpecification(IProperty property, XmlReader xmlReader)
        {
            ...
            switch (xmiType)
            {
                ...
                default:
                    var defaultLineInfo = xmlReader as IXmlLineInfo;
                    throw new NotSupportedException($"PropertyReader.ReadValueSpecification: {xmiType} at line:position {defaultLineInfo.LineNumber}:{defaultLineInfo.LinePosition}");
            }
        }
    }
}
